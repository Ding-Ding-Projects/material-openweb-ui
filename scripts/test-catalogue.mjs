...
function stubFetch(pages, { headers = {} } = {}) {
  return async (url) => {
    const key = new URL(url, 'https://ollama.com').pathname + new URL(url, 'https://ollama.com').search;
...
check('and every model across them is collected',
  threePages.models.map((m) => m.name).join(',') === 'a,b,c,d,e',
  threePages.models.map((m) => m.name).join(','));
check('the page count is measured rather than assumed',
  threePages.pages !== onePage.pages, 'both would be 1 if it were a literal');

const loop = await fetchWith({
  '/library': page(['a'], { next: '/library?page=2' }),
  '/library?page=2': page(['b'], { next: '/library' })
});
check('a page that links back to one already read does not loop forever',
  loop.pages === 2, String(loop.pages));
...
check('following pagination to its end reads as complete',
  threePages.completeness === 'complete', threePages.completeness);
check('and the verdict says what it is based on',
  /Followed every page/.test(threePages.basis || ''), threePages.basis);

check('a document with no pagination markup is UNVERIFIED, not complete',
  onePage.completeness === 'unverified', onePage.completeness);
check('and it says why it cannot tell',
  /cannot confirm whether more pages exist/.test(onePage.basis || ''), onePage.basis);
check('unverified does not claim the boolean either',
  onePage.complete === false, String(onePage.complete));
check('but the models it did read are still returned rather than withheld',
  onePage.models.length === 3);

const withPagination = await fetchWith({ '/library': page(['a', 'b'], { pagination: true }) });
check('a single page that DOES declare pagination and links no further is complete',
  withPagination.completeness === 'complete', withPagination.completeness);

const brokeMidway = await fetchWith({
  '/library': page(['a'], { next: '/library?page=2' })
  // page 2 is absent, so the fetch 404s
});
check('a page that fails makes the whole result incomplete',
  brokeMidway.completeness === 'incomplete', brokeMidway.completeness);
check('and it names the page that failed',
  /404/.test(brokeMidway.failure || ''), brokeMidway.failure);
check('what was already read is still returned rather than thrown away',
  brokeMidway.models.length === 1, String(brokeMidway.models.length));

const empty = await fetchWith({ '/library': '<html><body>nothing at all</body></html>' });
check('a page with no model links is a broken parser, not an empty catalogue',
  /markup changed/.test(empty.failure || ''), empty.failure);
...
check('and where it came from is recorded too', tagged.revisionFrom === 'ETag', tagged.revisionFrom);

const dated = await fetchWith({ '/library': page(['a']) }, { headers: { 'last-modified': 'Wed, 19 Aug 2026 09:00:00 GMT' } });
check('Last-Modified is used when there is no ETag', /2026/.test(dated.revision || ''), dated.revision);

const hashed = await fetchWith({ '/library': page(['a']) });
check('with no cache headers the body itself identifies the version',
  /^[0-9a-f]{16}$/.test(hashed.revision || ''), hashed.revision);
check('and it says the revision is a hash rather than a published version',
  /SHA-256/.test(hashed.revisionFrom || ''), hashed.revisionFrom);

const different = await fetchWith({ '/library': page(['a', 'b']) });
check('two different catalogues can be told apart',
  hashed.revision !== different.revision, hashed.revision + ' vs ' + different.revision);

check('every result records when it was fetched',
  /^\d{4}-\d{2}-\d{2}T/.test(onePage.fetchedAt), onePage.fetchedAt);
...
check('the heading follows the verdict rather than asserting one',
  /completeness === 'complete' \? 'Catalogue verified complete'/.test(core),
  '"verified complete" was printed for any fetch that read a link');
check('an unverified verdict does not get a success tick',
  /completeness === 'complete' \? 'check'/.test(surface));
check('the fetch is disclosed BEFORE it happens, not after',
...
check('the contract no longer claims only the local API is used',
  !/It speaks only Ollama's documented local HTTP API\./.test(contract),
  'that sentence was the comfortable one and the false one');
check('the contract states the one exception plainly',
  /Browsing the PUBLISHED catalogue is the one exception/.test(contract));
check('and names the three verdicts',
  /complete, incomplete or unverified/.test(contract));

console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. A completeness claim nobody measured is worse than none.');
  process.exit(1);
}
