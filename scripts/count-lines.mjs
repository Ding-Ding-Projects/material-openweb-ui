...
const SKIP_PATHS = ['design' + sep, 'docs' + sep + 'assets' + sep + 'fonts' + sep];
...
const GROUPS = [
	...
	{ name: 'Site (HTML)', match: (p) => p.startsWith('docs' + sep) && extname(p) === '.html' },
	{
		...
		match: (p) => p.startsWith('src' + sep) && extname(p) === '.svelte'
	},
	{
		name: 'Frontend (TS/JS)',
		match: (p) => p.startsWith('src' + sep) && ['.ts', '.js'].includes(extname(p))
	},
...
function forkDelta() {
	...
	if (!base) return null;
	try {
		const stat = execSync('git diff --numstat ' + base + ' HEAD', {
			encoding: 'utf8',
...
if (arg === '--json') {
	...
	for (const g of payload.groups)
		out.push('| ' + g.name + ' | ' + g.files + ' | ' + g.lines.toLocaleString('en-US') + ' |');
	out.push(
		'| **Total** | **' + payload.files + '** | **' + payload.total.toLocaleString('en-US') + '** |'
	);
	if (payload.fork) {
		out.push('');
		out.push(
			'Written in this fork since `' +
				payload.fork.base +
				'`: **+' +
				payload.fork.added.toLocaleString('en-US') +
				' / -' +
				payload.fork.removed.toLocaleString('en-US') +
				'** across ' +
				payload.fork.changedFiles +
				' files. Everything else in the tree is upstream Open WebUI.'
		);
	}
	out.push('');
	out.push(
		'Counted by `node scripts/count-lines.mjs --markdown`. Excludes ' +
			payload.excluded.join(', ') +
			'.'
	);
	...
	for (const g of payload.groups) {
		console.log(
			g.name.padEnd(w) +
				'  ' +
				String(g.files).padStart(5) +
				' files  ' +
				String(g.lines).padStart(8) +
				' lines'
		);
	}
	...
	console.log(
		'Total'.padEnd(w) +
			'  ' +
			String(payload.files).padStart(5) +
			' files  ' +
			String(payload.total).padStart(8) +
			' lines'
	);
	if (payload.fork) {
		console.log('');
