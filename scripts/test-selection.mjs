...
check(
	'toggling returns a new set rather than mutating',
	(() => {
...
check(
	'select-all under a filter selects only what is visible',
	filtered.size === 2 && S.has(filtered, 'b') && S.has(filtered, 'd'),
	[...filtered].join(',')
);
check(
	'it does NOT reach into the rest of the list',
	!S.has(filtered, 'a') && !S.has(filtered, 'c'),
	[...filtered].join(',')
);

const preexisting = S.selectAllInScope(S.create(['f']), FILTERED);
check(
	'a selection made outside the current view survives select-all',
	S.has(preexisting, 'f'),
	[...preexisting].join(',')
);
...
check(
	'clear-in-view clears only what is visible',
	cleared.size === 1 && S.has(cleared, 'f'),
	[...cleared].join(',')
);

const inverted = S.invertInScope(S.create(['b', 'f']), FILTERED);
check(
	'invert flips the visible items only',
...
check(
	'the total is the whole selection, not just the visible part',
	sum.selected === 6,
	String(sum.selected)
);
check('the visible count is right', sum.visible === 2, String(sum.visible));
check(
	'the HIDDEN count is right, and it is the number that matters',
	sum.hidden === 4,
	String(sum.hidden)
);
check(
	'a selection made entirely in view reports nothing hidden',
...
check(
	'the sentence states how many are not visible',
	/4 of them are not visible/.test(sentence),
	sentence
);
check(
	'the sentence names the action rather than saying "these items"',
	/delete/.test(sentence),
	sentence
);
check(
	'with nothing hidden it says so plainly rather than staying silent',
	/all of which are visible/.test(
		S.describeScope(S.summary(S.create(['b']), FILTERED, ALL), {
			verb: 'delete',
			noun: 'notification'
		})
	),
...
check(
	'with nothing selected it says nothing is selected',
	S.describeScope(S.summary(S.create(), FILTERED, ALL)) === 'Nothing is selected.'
);
check(
	'the singular reads correctly, since it is the common case',
	/1 notification, and 1 of them is not visible/.test(
		S.describeScope(S.summary(S.create(['a']), FILTERED, ALL), {
			verb: 'delete',
			noun: 'notification'
		})
	),
...
check(
	'identifiers that no longer exist are counted separately',
	stale.stale === 2,
	String(stale.stale)
);
check(
	'and are NOT counted as selected, so no action claims more than it can touch',
	stale.selected === 1,
	String(stale.selected)
);
check('pruning drops them', S.prune(S.create(['a', 'gone']), ALL).size === 1);
check('pruning keeps everything that still exists', S.prune(S.create(['a', 'f']), ALL).size === 2);
...
check(
	'a range covers both ends and everything between',
	[...range].sort().join(',') === 'b,c,d,e',
	[...range].sort().join(',')
);
check(
	'a range works backwards too',
	[...S.selectRange(S.create(), ALL, 'e', 'b')].sort().join(',') === 'b,c,d,e'
);
check(
	'a range is taken from the VIEW, so it cannot sweep in hidden rows',
	[...S.selectRange(S.create(), FILTERED, 'b', 'd')].sort().join(',') === 'b,d',
	[...S.selectRange(S.create(), FILTERED, 'b', 'd')].sort().join(',')
);
check(
	'a range with an endpoint that is not in view changes nothing',
...
check(
	'with everything in view selected, it reads as fully checked',
	S.summary(S.create(FILTERED), FILTERED, ALL).allInScopeSelected === true
);
check(
	'with some selected, it is neither checked nor empty',
	(() => {
...
check(
	'with none in view selected, it reads as empty even if others are',
	S.summary(S.create(['a']), FILTERED, ALL).noneInScopeSelected === true
);
check(
	'an empty view is not reported as fully selected',
...
check(
	'a word ending in a consonant plus y takes -ies, not -ys',
	S.plural(2, 'entry') === '2 entries',
	S.plural(2, 'entry')
);
check(
	'a word ending in a vowel plus y just takes -s',
	S.plural(2, 'day') === '2 days',
	S.plural(2, 'day')
);
check(
	'a sibilant takes -es',
	S.plural(2, 'match') === '2 matches' && S.plural(3, 'box') === '3 boxes'
);
check('an ordinary word takes -s', S.plural(2, 'item') === '2 items');
check('an irregular plural can be stated outright', S.plural(2, 'person', 'people') === '2 people');
check('zero is plural', S.plural(0, 'entry') === '0 entries');
check(
	'the scope sentence uses it',
	(() => {
		const sentence = S.describeScope(S.summary(S.create(['a', 'b']), [], ['a', 'b']), {
			verb: 'delete',
			noun: 'entry'
		});
		return /2 entries/.test(sentence) && !/entrys/.test(sentence);
	})(),
	S.describeScope(S.summary(S.create(['a', 'b']), [], ['a', 'b']), {
		verb: 'delete',
		noun: 'entry'
	})
);
...
check(
	'one bar is shared rather than a select-all per list',
	/export function bulkBar/.test(barSource)
);
check(
	'the bar renders the hidden-count warning itself, so no list can omit it',
...
const confirmLine =
	barSource
		...
		.find((l) => l.includes('sum.hidden ||') || l.includes('sum.hidden||')) || '';
check(
	'the out-of-view warning is a reason to confirm all by itself',
	/if \(\s*sum\.hidden\s*\|\|/.test(confirmLine),
	confirmLine.trim() || 'no such line'
);
check(
	'opting out of confirmation cannot opt out of the out-of-view warning',
	!/confirm !== false && \(action\.danger \|\| sum\.hidden\)/.test(barSource),
	confirmLine.trim()
);
check(
	'a routine confirmation can still be suppressed for an action with its own dialog',
	/action\.confirm !== false/.test(barSource)
);
check(
	'the bar pluralises through the shared helper rather than appending an s',
	/sel\.plural\(/.test(barSource) && !/noun \+ 's'/.test(barSource),
	(barSource.match(/noun \+ 's'/g) || []).join(',')
);
check(
	'the confirmation uses the shared sentence rather than its own wording',
	/describeScope\(sum/.test(barSource)
);
check(
	'stale selections are reported in the confirmation rather than silently skipped',
	/sum\.stale/.test(barSource)
);
check(
	'every list gets export in every format, not a JSON-only shortcut',
...
check(
	'the export dialog states its scope in the same words an action does',
	/describeScope\(sum/.test(exportBody),
	'export is not destructive, but it is the path where the original defect lived'
);
check(
	'and it warns when the file will contain rows that are not on screen',
...
if (failures) {
	console.error(
		failures + ' check(s) failed. This is the module where a mistake deletes something.'
	);
	process.exit(1);
}
console.log(
	'A selection survives a filter change, and no bulk action runs without saying what it cannot show you.'
);
process.exit(0);
