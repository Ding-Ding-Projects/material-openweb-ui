...
check(
	'a URI without parameters falls back to the usual defaults',
	defaulted.algorithm === 'SHA1' && defaulted.digits === 6 && defaulted.period === 30
);
...
check(
	'a URI with no secret is rejected',
	throws(() => mod.parseUri('otpauth://totp/Example:me?issuer=Example'))
);
check(
	'a non-otpauth URI is rejected',
	throws(() => mod.parseUri('https://example.test/'))
);
check(
	'an out-of-range digit count is rejected',
	throws(() => mod.parseUri('otpauth://totp/x?secret=JBSWY3DP&digits=9'))
);
check(
	'an unsupported algorithm is rejected',
	throws(() => mod.normaliseAlgorithm('MD5'))
);
...
check(
	'seconds remaining is within the period',
	(() => {
...
check(
	'the next-code peek matches the following window',
	(await mod.nextCode(secret, { atMs: base })) === justAfter
);
...
check(
	'grouping does not change the secret',
	mod.base32Decode(mod.groupSecret(secret)).length === mod.base32Decode(secret).length
);
...
c2.jump(3600);
check(
	'the clock being set an hour forward is detected',
	c2.watch.check() === 3600,
	String(c2.watch.check())
);
...
c3.jump(-900);
check(
	'the clock being set backward is detected too',
	c3.watch.check() === -900,
	String(c3.watch.check())
);
...
c4.jump(0.4);
check(
	'a fraction of a second is tolerated rather than reported as a jump',
	c4.watch.check() === 0,
	String(c4.watch.check())
);
...
c5.pass(10);
check(
	'after a jump is reported, ordinary time is ordinary again',
	c5.watch.check() === 0,
	String(c5.watch.check())
);
...
check(
	...
	(() => {
		...
		globalThis.fetch = () => {
			throw new Error('the clock watch must not reach the network');
		};
		try {
			const c = fakeClock();
			c.pass(5);
...
check(
	'the surface watches for a jump on every tick, not once at render',
	/watch\.check\(\)/.test(surface) && /setInterval/.test(surface)
);
check(
	'a network verdict ages rather than continuing to reassure',
...
check(
	'a jump invalidates the network verdict, which was measured against the old clock',
	/networkSkew = null;/.test(surface)
);
check(
	'the clock can be re-checked on demand rather than only at startup',
	/Check against network time now/.test(surface)
);
check(
	'the two questions are reported as two facts rather than blurred together',
...
check(
	'the page groups a code by a rule that covers seven and eight digits too',
