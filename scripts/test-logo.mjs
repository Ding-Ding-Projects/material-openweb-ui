...
function bmp(w, h) {
  return bytes(ascii('BM'), le32(0), le32(0), le32(54), le32(40), le32(w), le32(h), le16(1), le16(24));
}
function jpeg(w, h) {
  // A JFIF APP0 first, so the frame marker really has to be walked to.
  return bytes(
    [0xff, 0xd8, 0xff, 0xe0], [0x00, 0x10], ascii('JFIF'), [0, 1, 1, 0], le16(1), le16(1), [0, 0],
    [0xff, 0xc0], [0x00, 0x11], [8], [(h >> 8) & 255, h & 255], [(w >> 8) & 255, w & 255], [3]
  );
}
function webpVP8X(w, h, animated = false) {
  return bytes(ascii('RIFF'), le32(0), ascii('WEBP'), ascii('VP8X'), le32(10),
    [animated ? 0x02 : 0x00, 0, 0, 0], le24(w - 1), le24(h - 1));
}
function webpVP8(w, h) {
  return bytes(ascii('RIFF'), le32(0), ascii('WEBP'), ascii('VP8 '), le32(20),
    [0, 0, 0], [0x9d, 0x01, 0x2a], le16(w), le16(h));
}
function webpVP8L(w, h) {
  const packed = ((w - 1) & 0x3fff) | (((h - 1) & 0x3fff) << 14);
  return bytes(ascii('RIFF'), le32(0), ascii('WEBP'), ascii('VP8L'), le32(20), [0x2f], le32(packed));
}
...
check('identity carries a package id, executable, data directory and update feed',
  Boolean(logo.IDENTITY.packageId && logo.IDENTITY.executable && logo.IDENTITY.dataDirectory && logo.IDENTITY.updateFeed));
...
check('every identity value is a literal, never derived from a setting',
  identityBlock.length > 0 && !/\$\{|\+|state\.|settings|displayName|logo/.test(
    identityBlock.replace(/\/\/[^\n]*/g, '').replace(/material-openweb-ui|material-openwebui/g, '')),
  identityBlock.replace(/\s+/g, ' ').slice(0, 100));

const shell = readFileSync(join(ROOT, 'electron', 'main.ts'), 'utf8');
check('the desktop shell does not build a path from a display name',
...
for (const [label, data, w, h, format] of CASES) {
  let got = null;
  let err = null;
  try { got = logo.readDimensions(data); } catch (e) { err = e.message; }
  check(label + ' reports its real size',
    got && got.width === w && got.height === h && got.format === format,
    err || JSON.stringify(got));
}

check('a top-down BMP with a negative height is read as positive',
  logo.readDimensions(bmp(64, -32 >>> 0)).height > 0);
check('an animated GIF is counted as more than one frame', logo.readDimensions(gif(32, 32, 5)).frames === 5);
check('an APNG is counted as more than one frame', logo.readDimensions(png(32, 32, { apngFrames: 12 })).frames === 12);
...
check('acceptance reports what the bytes actually were',
  good.sniffed === 'PNG image' && Boolean(good.evidence), JSON.stringify(good.sniffed));
...
check('an animated image is told that only the first frame is used',
  /only the first is used/.test(animated.note || ''), animated.note);
...
check('the refusal says what the bytes really are, not what the name claimed',
  /PDF/.test(renamed.reason), renamed.reason);
check('the refusal points out that an extension is only a label',
  /extension is a label/.test(renamed.reason), renamed.reason);
check('a refusal applies nothing at all: there is no partial result',
...
check('a decompression bomb is refused on its declared size, before any decoding',
  huge.ok === false && /limit on either side/.test(huge.reason), huge.reason);

const wide = logo.inspect(png(4000, 4000), 'big.png');
check('an image inside the side limit but over the pixel budget is refused',
  wide.ok === false && /megapixels/.test(wide.reason), wide.reason);
check('the pixel refusal states the memory it would have taken',
  /MB of memory/.test(wide.reason || ''), wide.reason);

const oversize = logo.inspect(new Uint8Array(5 * 1024 * 1024), 'fat.png');
check('a file over the byte limit is refused', oversize.ok === false && /the limit is/.test(oversize.reason));
...
check('a PNG signature with no IHDR is refused',
  truncated.ok === false && /IHDR/.test(truncated.reason), truncated.reason);
...
check('verification reports the size it read back', okVariant.width === 64 && okVariant.height === 64);

const emptyVariant = logo.verifyVariant(new Uint8Array(0));
check('an encoder that returned nothing is caught',
  emptyVariant.ok === false && /no bytes/.test(emptyVariant.reason), emptyVariant.reason);

const wrongVariant = logo.verifyVariant(gif(64, 64));
check('an encoder that silently substituted a format is caught',
  wrongVariant.ok === false && /GIF/.test(wrongVariant.reason), wrongVariant.reason);

check('variants cover the sizes a platform actually asks for',
  logo.VARIANT_SIZES.includes(16) && logo.VARIANT_SIZES.includes(32) && logo.VARIANT_SIZES.includes(256));
...
check('a crop is pulled back inside the image rather than running off it',
  c1.x === 0 && c1.y === 0 && c1.size === 1, JSON.stringify(c1));
check('the crop size is preserved when the offset has to give way',
  logo.normaliseCrop({ x: 0.95, y: 0, size: 0.5 }, 100, 100).size === 0.5);
check('a missing crop becomes the whole image rather than nothing',
  logo.normaliseCrop(undefined, 64, 64).size === 1);
check('a nonsense crop is clamped, not trusted',
...
const { contrast } = await import(pathToFileURL(join(ROOT, 'docs', 'assets', 'js', 'colour.js')).href);
const dark = logo.contrastWarnings([0.05, 0.05, 0.06],
  [{ label: 'dark title bar', rgb: [0.08, 0.07, 0.09] }, { label: 'light title bar', rgb: [1, 1, 1] }], contrast);
check('a dark mark on a dark surface is warned about', dark.length === 1, JSON.stringify(dark));
check('the warning names the surface and the ratio',
  /dark title bar/.test(dark[0]) && /:1/.test(dark[0]), dark[0]);

const fine = logo.contrastWarnings([0, 0, 0],
  [{ label: 'light title bar', rgb: [1, 1, 1] }], contrast);
...
check('every preset renders to standalone SVG',
  logo.PRESETS.every((p) => logo.presetSvg(p).startsWith('<svg') && logo.presetSvg(p).includes('</svg>')));
check('a preset fetches nothing from anywhere',
