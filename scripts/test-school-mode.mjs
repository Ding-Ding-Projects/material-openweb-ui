...
check(
	'the application declares which settings School mode hides',
	/PLAYFUL_SETTINGS\s*=/.test(i18n)
);
...
check(
	'School mode forces English presentation',
	/function mode\(\)[\s\S]{0,220}schoolOn\(\)[\s\S]{0,80}return 'English'/.test(i18n)
);
check(
	'School mode flattens the funny level',
...
for (const [label, rel] of [
	['application', 'app/js/app.js'],
	['site', 'docs/assets/js/app.js']
]) {
	const src = read(rel);
	const fn = (src.match(/function maybeDimSum\(\)[\s\S]*?\n}/) || [''])[0];
	check(
		'the ' + label + ' dim sum surprise checks School mode',
		/school/i.test(fn),
		fn ? 'no school check in maybeDimSum' : 'maybeDimSum not found'
	);
	check(
		'the ' + label + ' dim sum surprise returns before drawing when it is on',
...
check(
	'the funny-level disclosure is gated',
	/isPlayfulHidden\('funnyEn'\)[\s\S]{0,120}\?\s*null/.test(settings)
);
check('the vocabulary card is gated', /isPlayfulHidden\('vocab'\)\s*\?\s*null/.test(settings));
check(
	'the Cantonese voice picker is gated',
	/isPlayfulHidden\('language'\)[\s\S]{0,200}voiceEn/.test(settings)
);

// The on-state explanation must not enumerate the hidden capabilities.
const onState = (settings.match(/school\.on[\s\S]{0,900}?'It is on'[\s\S]{0,900}?\)\)\)/) || [
	''
])[0];
...
check(
	'the site has a control that turns School mode on',
	/Turn it on/.test(sitePages) && /school: \{ on: true/.test(sitePages)
);
check(
	'turning it off needs the PIN it was turned on with',
	/value !== school\.pin/.test(sitePages)
);
check('a PIN too short to be one is refused', /value\.length < 4/.test(sitePages));
check(
	'the disclaimer reaches the page rather than sitting in the string table',
	/i18n\.t\('set\.schoolWhy'\)/.test(sitePages)
);
check(
	'and it still says this is not a security boundary',
	/not a security boundary/.test(
		readFileSync(join(ROOT, 'docs', 'assets', 'js', 'i18n.js'), 'utf8')
	)
);

check(
	'the playful features are marked on the feature itself',
	(siteContent.match(/playful: true/g) || []).length >= 5,
	'a list kept somewhere else is maintained in one place and forgotten in three'
);
check(
	'there is one shared filter rather than one per surface',
...
check('the command palette uses it for features', /withoutHidden\(FEATURES\)/.test(sitePalette));
...
check(
	'the article count is taken from the filtered list, not the whole one',
