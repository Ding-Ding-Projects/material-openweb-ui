...
const ROOTS = [join(ROOT, 'docs', 'assets', 'js'), join(ROOT, 'app', 'js')];
...
if (!files.length) {
	console.error(
		'FAIL: no modules found to parse. The layout changed and this guard is checking nothing.'
	);
	process.exit(1);
}
...
if (failures.length) {
	...
	for (const f of failures) {
		console.error(
			'FAIL: ' +
				f.rel +
				(f.kind === 'import'
					? ' has an import that does not resolve.'
	...
	console.error(
		[
			syntax ? syntax + ' syntax error(s)' : null,
			unresolved ? unresolved + ' unresolvable import(s)' : null
		]
			.filter(Boolean)
			.join(' and ') + '. Either one renders as a blank page.'
	);
	process.exit(1);
}
...
for (const dir of OWN) {
	for (const file of everyOwnFile(join(ROOT, dir))) {
		...
		for (const m of text.matchAll(PATTERN_CALL)) {
			...
			try {
				source = JSON.parse(
					'"' + m[1].split(String.fromCharCode(92, 34)).join(String.fromCharCode(92, 92, 34)) + '"'
				);
			} catch {
