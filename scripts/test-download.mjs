...
const anyLinkAt = Math.min(
  ...[src.indexOf("h('a', {"), src.indexOf('f.url')].filter((i) => i > -1).concat([Number.MAX_SAFE_INTEGER])
);
check('the warning block exists', warningAt > -1);
check('the warning is built BEFORE any download link',
  warningAt > -1 && warningAt < anyLinkAt, warningAt + ' vs ' + anyLinkAt);
// Scoped to the render call itself. An earlier version compared the first
// occurrence of each name anywhere in the file, and "add(list," appears well
// before the render — so it was measuring two unrelated lines and would have
// passed whatever order the page was actually assembled in.
const renderCall = src.slice(src.indexOf('add(wrap,'));
check('the render call was found', renderCall.startsWith('add(wrap,'));
check('the warning is added to the page before the list of files',
  renderCall.indexOf('warning,') > -1 &&
  renderCall.indexOf('warning,') < renderCall.indexOf('list,'),
  renderCall.split(String.fromCharCode(10)).slice(0, 8).map((l) => l.trim()).join(' | '));

console.log('');
console.log('what it says');

check('it names what Windows will actually show', /SmartScreen/i.test(src));
check('it names what macOS will actually show',
  /macOS/.test(src) && /cannot be checked/i.test(src));
check('it says the warning is correct rather than a false alarm',
  /Both are correct/i.test(src) && /Neither is a false alarm/i.test(src));
check('it gives the real reason rather than a vague one',
  /annual fee|certificate is an annual/i.test(src));
check('it says the signing is permanently absent, not merely pending',
  /permanently/i.test(src));
...
check('the stripped prose still contains the page text, so the checks above mean something',
  /SmartScreen/.test(shown) && /annual fee/.test(shown) && shown.length > 1200,
  String(shown.length) + ' characters left');
check('a page that really did advise ignoring the warning would still be caught',
...
check('the checksum is not overclaimed as proof of origin',
  /does not prove where it came from/i.test(src));
check('an empty release list says so rather than listing nothing quietly',
  /Nothing has been released yet/.test(src));
check('and it explains why an imaginary link would be worse',
  /missing file|does not exist/i.test(src));

console.log('');
console.log('bilingual');

check('the warning is given in Cantonese too, not only English',
  /cjk/.test(src) && /簽名/.test(src));
check('the Cantonese says the warnings are correct as well',
  /兩個警告都係啱嘅/.test(src));

console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. A download page that softens this is teaching a dangerous habit.');
  process.exit(1);
}
console.log('The warning comes first, says what will actually happen, and never advises clicking past it.');
process.exit(0);
