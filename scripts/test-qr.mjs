...
for (const text of CASES) {
	...
	check(
		'round trips ' + JSON.stringify(text.slice(0, 44)) + (text.length > 44 ? '…' : ''),
		got === text,
		err || JSON.stringify(String(got).slice(0, 60))
	);
}
...
check(
	...
	m.modules.length === m.size && m.modules.every((r) => r.length === m.size)
);
check(
	'the size follows the version formula',
	m.size === m.version * 4 + 17,
	m.size + ' vs ' + (m.version * 4 + 17)
);
check('a mask was chosen from the eight', m.mask >= 0 && m.mask <= 7, String(m.mask));
check(
	'the mask written into the format information is the one that was applied',
	(() => {
		const copy = { ...m, mask: 99 };
		try {
			return qr.decode(copy) === 'otpauth://totp/Example:me?secret=JBSWY3DPEHPK3PXP';
		} catch {
			return false;
		}
	})(),
...
check(
	'every module is a boolean, with none left unset',
...
check(
	'the SVG fetches nothing from anywhere',
	!/https?:/.test(svg.replace('http://www.w3.org/2000/svg', ''))
);
check(
	'the quiet zone is honoured',
	svg.includes('width="' + (m.size + 8) * 5 + '"'),
	'expected ' + (m.size + 8) * 5
);
check(
	'it is true black on true white rather than themed',
	svg.includes('#FFFFFF') && svg.includes('#000000') && !/var\(--/.test(svg)
);
check(
	'every dark module is drawn',
...
check(
	'something far too long is refused',
	throws(() => qr.encode('x'.repeat(5000)))
);
