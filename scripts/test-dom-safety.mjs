...
if (problems.length) {
	...
	for (const p of problems) {
		console.error(
			'FAIL: ' + p.file + ':' + p.line + ' — ' + p.target + '.append(...) can receive null.'
		);
	...
	console.error('');
	console.error(
		problems.length + ' unsafe append(s). Each one renders the literal word "null" on screen.'
	);
	process.exit(1);
}
...
if (!/c === null \|\| c === undefined \|\| c === false/.test(domSrc)) {
	console.error(
		'FAIL: dom.js `add` no longer filters null/undefined/false, so it does not fix what this guard is about.'
	);
	process.exit(1);
}
