...
function reason(fn) { try { fn(); return ''; } catch (e) { return e.message; } }
...
const good = {
  schema: 1,
  timeZone: LONDON,
  rules: [{ id: 'night', startTime: '20:00', endTime: '07:00', days: [5], patch: { theme: 'dark' } }]
};
check('a well-formed schedule validates', s.validate(good).rules.length === 1);
check('a document from a different schema version is refused',
...
check('a schedule with no timezone is refused', throws(() => s.validate({ ...good, timeZone: '' })));
check('an invented timezone is refused', throws(() => s.validate({ ...good, timeZone: 'Middle/Earth' })));
check('a rule with no identifier is refused',
  throws(() => s.validateRule({ startTime: '01:00', endTime: '02:00', patch: { theme: 'dark' } })));
check('two rules sharing an identifier are refused',
  throws(() => s.validate({ ...good, rules: [good.rules[0], { ...good.rules[0] }] })));
check('a rule that changes a setting outside the allowed list is refused',
  throws(() => s.validateRule({ id: 'x', patch: { ollamaHost: 'http://elsewhere' } })));
check('the refusal names the setting it would not change',
  /ollamaHost/.test(reason(() => s.validateRule({ id: 'x', patch: { ollamaHost: 'http://elsewhere' } }))));
check('a rule that changes nothing is refused', throws(() => s.validateRule({ id: 'x', patch: {} })));
check('a rule covering no days is refused',
  throws(() => s.validateRule({ id: 'x', days: [], patch: { theme: 'dark' } })));
check('a weekday outside 0 to 6 is refused',
  throws(() => s.validateRule({ id: 'x', days: [7], patch: { theme: 'dark' } })));
check('a rule ending before it starts is refused',
  throws(() => s.validateRule({ id: 'x', startDate: '2026-05-01', endDate: '2026-04-01', patch: { theme: 'dark' } })));
check('a malformed time is refused', throws(() => s.validateRule({ id: 'x', startTime: '25:00', patch: { theme: 'dark' } })));
check('a malformed date is refused', throws(() => s.validateRule({ id: 'x', startDate: '1st May', patch: { theme: 'dark' } })));
check('more rules than the limit are refused',
  throws(() => s.validate({ ...good, rules: Array.from({ length: 65 }, (_, i) => ({ id: 'r' + i, patch: { theme: 'dark' } })) })));
check('duplicate weekdays are collapsed rather than kept',
...
const day = s.validateRule({ id: 'day', startTime: '09:00', endTime: '17:00', patch: { theme: 'light' } });

check('inside the window it is active', s.evaluate(day, instantFor(LONDON, '2026-06-10T12:00'), LONDON).active);
check('at the start minute it is active', s.evaluate(day, instantFor(LONDON, '2026-06-10T09:00'), LONDON).active);
check('at the end minute it is already over, so the two ends do not both fire',
  !s.evaluate(day, instantFor(LONDON, '2026-06-10T17:00'), LONDON).active);
check('a minute before the start it is not active',
...
const always = s.validateRule({ id: 'always', startTime: '00:00', endTime: '00:00', patch: { theme: 'dark' } });
check('an equal start and end means all day, not never',
...
const friNight = s.validateRule({ id: 'fri', startTime: '20:00', endTime: '07:00', days: [5], patch: { theme: 'dark' } });

check('Friday evening is inside the window',
  s.evaluate(friNight, instantFor(LONDON, '2026-06-12T22:30'), LONDON).active);
check('Saturday morning is STILL inside it, without Saturday being selected',
  s.evaluate(friNight, instantFor(LONDON, '2026-06-13T03:00'), LONDON).active);
check('the window is attributed to the Friday it began on',
...
check('Saturday EVENING is not inside it, because Saturday is not selected',
  !s.evaluate(friNight, instantFor(LONDON, '2026-06-13T22:30'), LONDON).active);
check('Friday morning is not inside it: that would belong to Thursday night',
  !s.evaluate(friNight, instantFor(LONDON, '2026-06-12T03:00'), LONDON).active);
check('the refusal explains which day the window would have begun on',
  /Thursday/.test(s.evaluate(friNight, instantFor(LONDON, '2026-06-12T03:00'), LONDON).reason || ''),
  s.evaluate(friNight, instantFor(LONDON, '2026-06-12T03:00'), LONDON).reason);
check('midday Friday, between the two halves, is outside',
...
const ranged = s.validateRule({
  id: 'ranged', startTime: '22:00', endTime: '02:00',
  startDate: '2026-06-12', endDate: '2026-06-12', patch: { theme: 'dark' }
});
check('a one-day range covers the night that began inside it',
  s.evaluate(ranged, instantFor(LONDON, '2026-06-13T01:00'), LONDON).active);
check('a one-day range does not cover the night that began after it',
...
const skipped = s.validateRule({ id: 'skipped', startTime: '01:15', endTime: '01:45', patch: { theme: 'dark' } });
...
check('the zone really does skip an hour on that date',
...
const repeated = s.validateRule({ id: 'repeated', startTime: '01:30', endTime: '03:00', patch: { theme: 'dark' } });
...
check('the zone really does repeat an hour on that date',
...
check('the window is open on the first pass through the repeated hour',
  s.evaluate(repeated, firstPass, LONDON).active);
check('it is still simply open on the second pass, not reopened',
  s.evaluate(repeated, secondPass, LONDON).active);
check('it closes at the wall-clock end time, once',
...
const doc = s.validate({
  ...
  timeZone: LONDON,
  rules: [
    { id: 'holiday', startTime: '18:00', endTime: '23:00', startDate: '2026-06-10', endDate: '2026-06-10', patch: { theme: 'light' } },
...
check('with only the broad rule in force, it wins by default', midday.settings.theme === 'light', JSON.stringify(midday.settings));

const evening = s.resolve(doc, instantFor(LONDON, '2026-06-11T20:00'), LONDON);
check('a time-limited rule beats an all-day one listed after it',
  evening.settings.theme === 'dark', JSON.stringify(evening.settings));
check('the winning rule is named, not left to be guessed at', evening.because.theme === 'evenings', evening.because.theme);
check('every rule in force is reported, not only the winner', evening.applied.length === 2, JSON.stringify(evening.applied));

const holiday = s.resolve(doc, instantFor(LONDON, '2026-06-10T20:00'), LONDON);
check('a date-limited rule beats a merely time-limited one listed after it',
  holiday.settings.theme === 'light' && holiday.applied.length === 3, JSON.stringify(holiday.settings));
check('and it says which rule did it', holiday.because.theme === 'holiday', holiday.because.theme);

const tie = s.validate({
  schema: 1, timeZone: LONDON,
  rules: [
    { id: 'first', startTime: '09:00', endTime: '17:00', patch: { theme: 'light' } },
    { id: 'second', startTime: '09:00', endTime: '17:00', patch: { theme: 'dark' } }
  ]
});
check('equally specific rules are resolved by position, later winning',
  s.resolve(tie, instantFor(LONDON, '2026-06-11T12:00'), LONDON).because.theme === 'second');

check('specificity is counted rather than guessed at',
  s.specificity({ startDate: '2026-01-01', days: [1], startTime: '09:00', endTime: '17:00' }) === 4 &&
  s.specificity({ days: [0, 1, 2, 3, 4, 5, 6], startTime: '00:00', endTime: '00:00' }) === 0);
...
check('a midnight-crossing rule explains itself where the rule is, not in a help page',
  /crosses midnight/.test(described), described);
check('a midnight-crossing description names the following day explicitly',
  /Saturday/.test(described), described);
check('the daylight-saving behaviour is documented in both languages',
  s.DST_NOTE.en.length > 100 && s.DST_NOTE.zh.length > 20);
check('the note says wall clock rather than offset',
  /wall-clock/.test(s.DST_NOTE.en) && /not offsets from UTC/.test(s.DST_NOTE.en));
check('an empty schedule carries the schema version and a real zone',
  s.empty('Europe/London').schema === s.SCHEMA_VERSION && s.empty('Europe/London').timeZone === 'Europe/London');

console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. A schedule that is wrong at an edge is wrong twice a year, silently.');
  process.exit(1);
}
console.log('Midnight, both daylight-saving boundaries and the precedence rule all behave as documented.');
process.exit(0);
