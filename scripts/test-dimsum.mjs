...
check('a second draw in the same load produces nothing, even when the odds say yes',
  second === null, JSON.stringify(second));

let extra = 0;
for (let i = 0; i < 50; i++) if (D.draw({ schoolOn: false, random: always })) extra++;
check('and it stays nothing however many times it is called', extra === 0, String(extra) + ' extra');
...
check('and a refused draw does not consume the once-per-load guard, or one unlucky launch would silence every later one',
  D.hasFired() === false);
check('so it can still fire afterwards', Boolean(D.draw({ schoolOn: false, random: always })));
check('the chance is one in ten, as stated', D.CHANCE === 0.10, String(D.CHANCE));
...
check('it produces nothing rather than something marked as hidden',
  underSchool === null, 'a note explaining what was hidden would name the thing being hidden');
check('and it does not consume the guard either, so turning School mode off later still works',
  D.hasFired() === false);
check('with School mode off it fires normally',
...
let consulted = 0;
D.resetForTest();
D.draw({ schoolOn: true, random: () => { consulted++; return 0; } });
...
check('every dish is bilingual', D.DISHES.every((d) => d.en && d.zh));
check('every dish states its photograph or states that there is none',
  D.DISHES.every((d) => Object.prototype.hasOwnProperty.call(d, 'photo')),
  'an absent field is a gap; an explicit null is a fact');
check('at least one dish has no published photograph, so the case is real rather than theoretical',
  D.DISHES.some((d) => d.photo === null));
check('no photograph is a path inside this repository',
  D.DISHES.every((d) => !d.photo || (!d.photo.includes('..') && !d.photo.startsWith('/') && !d.photo.includes('assets/'))),
...
check('a dish with a photograph says where it lives',
  withPhoto && /public catalogue/.test(withPhoto.provenance), withPhoto && withPhoto.provenance);
check('a dish WITHOUT one says so instead of saying the same thing',
  withoutPhoto && /no published photograph/.test(withoutPhoto.provenance),
  withoutPhoto && withoutPhoto.provenance);
check('the two sentences really are different, or the absence is invisible',
  withPhoto && withoutPhoto && withPhoto.provenance !== withoutPhoto.provenance);
check('the missing-photograph sentence says nothing is substituted locally',
...
check('there is no setting to switch it off',
...
check('neither surface keeps its own copy of the dish list',
  !/const DIM_SUM\s*=/.test(appJs) && !/const DIM_SUM\s*=/.test(siteJs),
  'two lists is two lists, and the second one goes stale');
check('both surfaces draw from the shared module',
  /dimsum\.draw\(/.test(appJs) && /dimsum\.draw\(/.test(siteJs));
check('it is delivered as a notification, which already promises not to block or steal focus',
  /ui\.notify\(/.test(appJs) && /ui\.notify\(/.test(siteJs));
check('it auto-dismisses rather than waiting to be closed',
  /duration:\s*\d+/.test(appJs) && /duration:\s*\d+/.test(siteJs));
check('the module never loads a remote image itself',
  !/new Image|fetch\(|<img/.test(dimsumSrc));
...
console.log('At most once a load, absent under School mode, and honest about the dishes it has no photograph of.');
process.exit(0);
