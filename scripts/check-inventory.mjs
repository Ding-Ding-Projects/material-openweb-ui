...
if (!existsSync(INVENTORY)) {
	console.error(
		'FAIL: INVENTORY.md is missing. The completeness inventory is the contract; without it there is nothing to check against.'
	);
	process.exit(1);
}
if (!existsSync(CATALOGUE)) {
	console.error(
		'FAIL: docs/assets/js/content.js is missing, so the catalogue cannot be compared with the inventory.'
	);
	process.exit(1);
}
...
if (!rows.length) {
	console.error(
		'FAIL: INVENTORY.md parsed to zero feature rows. Either the table shape changed or the inventory was emptied.'
	);
	process.exit(1);
}
...
if (!catalogue.length) {
	fail(
		'Parsed zero features out of content.js. The catalogue shape changed, and this guard is now checking nothing.'
	);
}
...
for (const f of catalogue) {
	if (!rowById.has(f.id)) {
		fail(
			'Feature "' +
				f.id +
				'" is in the catalogue but has no row in INVENTORY.md. A feature that vanishes from the inventory is exactly what this guard exists to catch.'
		);
	}
}
...
for (const r of rows) {
	if (!catById.has(r.id)) {
		fail(
			'INVENTORY.md row "' +
				r.id +
				'" names a feature that is not in the catalogue. Either it was deleted and the row is stale, or the id is wrong.'
		);
	}
}
	...
	if (claimsCode && !hasAnchor) {
		fail(
			'Row "' +
				...
				' / app: ' +
				r.app +
				' but names no implementation anchor. A claim with nothing behind it is the thing an inventory is supposed to prevent.'
		);
		continue;
	}
	...
	if (!claimsCode && hasAnchor) {
		fail(
			'Row "' +
				...
				' / app: ' +
				r.app +
				' yet carries an anchor (' +
				r.anchor +
				'). Either it is actually built and the status is wrong, or the anchor is fiction.'
		);
		continue;
	}
	...
	if (hash === -1) {
		fail(
			'Row "' +
				...
				'" anchor "' +
				r.anchor +
				'" is not in path#needle form, so it cannot be checked.'
		);
		continue;
	}
	...
	if (!containsExact(body, needle)) {
		...
		fail(
			'Row "' +
				...
				'#' +
				needle +
				', but that file does not contain "' +
				needle +
				'"' +
				(softHit
					? ' on an identifier boundary — it only appears as part of a longer name, which is what a rename looks like.'
					: '. A rename or deletion that skipped the inventory looks exactly like this.')
		);
		continue;
	}
	...
	if (r.site !== f.site) {
		fail(
			'Feature "' +
				...
				'" is ' +
				f.site +
				' in the catalogue the site renders, but ' +
				r.site +
				' in INVENTORY.md. The page must never claim more than the inventory does.'
		);
	}
	if (r.app !== f.app) {
		fail(
...
console.log(
	'Inventory is consistent: every catalogued feature has a row, every row names a real feature, every shipped claim resolves, and the page agrees with the list.'
);
process.exit(0);
