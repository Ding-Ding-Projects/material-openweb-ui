...
check(
	'a data-ap identifier wins and is reported as exact',
	(() => {
...
check(
	'a derived key is reported as inexact rather than presented as certain',
	(() => {
...
const sheet = () =>
	(document.getElementById('mowui-appearance') || { textContent: '' }).textContent;

check('a base override is written as a rule', /border-radius: 2px/.test(sheet()), sheet());
check(
	'a state override is written against its own selector',
	/:hover/.test(sheet()) && /#00ff00/.test(sheet()),
	sheet()
);
check(
	'an override wins over the stylesheet it is overriding',
	(sheet().match(/!important/g) || []).length >= 3,
	sheet()
);

check(
	'one property resets without touching its neighbours',
	(() => {
...
check(
	'one element resets without touching the others',
	(() => {
...
check(
	'resetting everything leaves nothing behind',
	(() => {
		ap.resetAll();
		return sheet() === '';
	})(),
...
check(
	'a file without the schema tag is refused',
	throws(() => ap.importJson(JSON.stringify({ overrides: { a: { base: { color: 'red' } } } })))
);
check(
	'something that is not JSON is refused',
	throws(() => ap.importJson('not json at all'))
);

const result = ap.importJson(
	JSON.stringify({
...
check(
	'a value that would close its own rule is dropped',
	!/display: none/.test(sheet()) && !sheet().includes('body {'),
	sheet()
);
...
check(
	'the count of what was dropped is reported rather than swallowed',
	result.dropped >= 4,
...
check(
	'a derived key warns the user it may match more than one element',
	/no appearance identifier/i.test(src)
);
check(
	'the typeface list says which source it came from',
	/cannot enumerate|not granted/i.test(src) && /source: 'bundled'|source: 'system'/.test(src)
);
check('the editor is itself an editable element', /'data-ap': 'appearance-editor'/.test(src));
check('focus returns to the element that was being edited', /target\.focus\(\)/.test(src));
check(
	'the picker is a continuous field, not only swatches',
	/cp__field/.test(picker) && /pointermove/.test(picker)
);
check(
	'the picker field is operable from the keyboard',
	/ArrowLeft/.test(picker) && /ArrowUp/.test(picker)
);
check(
	'an out-of-gamut value warns before it is applied, not after',
	/Outside what sRGB can show/.test(picker)
);
check('the CMYK conversion admits it has no colour profile', /no colour profile/i.test(picker));
check(
	'every interaction state is offered, not only the resting one',
	ap.STATES.length >= 5 && ap.STATES.some((s) => s.suffix.includes(':focus-visible'))
);
check(
	'keyboard focus is styled rather than plain focus',
...
console.log(
	'Appearance keys are stable, resets are surgical, and imported overrides are screened.'
);
process.exit(0);
