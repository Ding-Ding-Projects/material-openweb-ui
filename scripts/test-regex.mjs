...
check('plain text is the default and regex is opted into',
  /useRegex:\s*false/.test(regexSrc), 'a field that starts in regex mode surprises everyone');
check('the builder is a popover anchored to an element, not a free-floating dialog',
  /popover\(/.test(regexSrc));
check('the anchor is checked for being in the document before it is used',
...
check('and the field\'s own button is the fallback',
  /opts\.anchor && opts\.anchor\.isConnected\) \? opts\.anchor : rxBtn/.test(regexSrc));

check('the builder offers guided tokens as well as a raw pattern',
  /rx\.pattern/.test(regexSrc) && /flags/.test(regexSrc));
check('validity is shown live rather than on submit', /live\(\)/.test(regexSrc));
check('a pattern is evaluated here and never sent anywhere',
...
check('it opens the builder by calling it, not by clicking a button it never mounted',
  /f\.openBuilder\(\)/.test(paletteSrc) && !/querySelector\('\.rx-btn'\)\.click\(\)/.test(paletteSrc),
  'synthesising a click on an off-DOM button is what anchored the panel to nothing');
check('it passes its own in-document button as the anchor',
  /anchor:\s*rxBtn/.test(paletteSrc));
check('the palette keeps its own query state rather than sharing another field\'s',
  /const state = \{ query: '', pattern: '', flags: 'i', useRegex: false \}/.test(paletteSrc));
...
check('no surface hand-rolls a search field without a builder beside it',
  homemade.length === 0, homemade.join(' | '));

const usingSearchField = files.filter((f) => /searchField\(/.test(readFileSync(f, 'utf8')));
check('several surfaces use the shared field, so the check above is not vacuous',
  usingSearchField.length >= 5, String(usingSearchField.length) + ' files');
...
console.log('Every builder opens beside the field it belongs to, and no pattern leaves the machine.');
process.exit(0);
