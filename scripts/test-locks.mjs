...
check(
	'a password is stored as a hash, never as the password',
...
check(
	'listing locks never exposes the secret or the hash',
	/secret:\s*undefined[\s\S]{0,40}hash:\s*undefined/.test(core)
);
check(
	'each lock is keyed independently, with no master credential',
	/\[id\]:\s*lock/.test(coreCode) && !/masterCredential|masterKey|MASTER_/.test(coreCode)
);
check('one-time-code verification allows a small clock skew', /\[-1, 0, 1\]/.test(core));
...
check(
	'rule 1: clearWait never removes or rewrites a credential',
	!/hash|secret|remove\(|create\(/.test(clearWait),
	clearWait.replace(/\s+/g, ' ').slice(0, 90)
);
check(
	'rule 1: winning is described as clearing the wait and nothing else',
	/only the wait/i.test(ui)
);
check(
	'rule 1: no session or cookie is minted anywhere in the ladder',
	!/document\.cookie|sessionStorage\.setItem|signIn\(|authenticate\(/.test(uiCode)
);

// 2. never refunds the attempt budget
check(
	'rule 2: the ladder never resets the attempt counter',
...
check(
	'rule 3: a spend is refused once the cap is reached',
	/if \(!b\.remaining\) return false/.test(core)
);
check(
	'rule 3: the interface refuses to open the ladder with no budget left',
...
check(
	'rule 4: the lockout escalates exponentially and is capped',
	/2 \*\* \(record\.count - 2\)/.test(tryUnlock) && /Math\.min\(/.test(tryUnlock)
);
check('rule 4: the ladder does not touch the escalation curve', !/LADDER|ladder/i.test(tryUnlock));

// 5. single-use challenges
check('rule 5: each challenge carries a nonce', /const thisNonce = nonce/.test(ui));
check('rule 5: a stale challenge grades nothing', /if \(thisNonce !== nonce\) return/.test(ui));
check(
	'rule 5: the nonce is consumed before grading',
...
check(
	'a timed round cannot be won faster than it lasts',
	/Date\.now\(\) - startedAt < ROUND_MS/.test(ui)
);
check('a mole counts only where it was actually visible', /if \(i !== live\) return/.test(ui));
...
check(
	'School mode removes the dim-sum rung rather than skipping it with a message',
...
check(
	'the disclosure never claims to secure anything',
	/not encryption/i.test(core) &&
		!/\bsecures?\b|\bprotects? your\b/i.test(core.replace(/does not protect[^.]*\./gi, ''))
);
check('the recovery route is documented rather than hidden', /Support Tickets/.test(ui));
check(
	'the support desk states plainly that nothing is sent anywhere',
	/Nothing is sent anywhere/.test(ui)
);
check(
	'the support desk never deletes anything itself',
	/never deletes anything|your action/i.test(ui) && !/rmdir|unlink|clearAll\(\)/.test(uiCode)
);
check(
	'local grading is admitted rather than implied to be server-side',
