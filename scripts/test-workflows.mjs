...
const PUBLISHES =
	/pypa\/gh-action-pypi-publish|docker\/build-push-action|softprops\/action-gh-release|ncipollo\/release-action|actions\/create-release|npm publish|docker push|gh release create|twine upload/;
...
for (const file of files) {
	...
	const jobs = [...jobsBlock.matchAll(/^  ([a-zA-Z0-9_-]+):$/gm)].map((m) => ({
		name: m[1],
		at: m.index
	}));
	for (let i = 0; i < jobs.length; i++) {
		const body = jobsBlock.slice(
			jobs[i].at,
			i + 1 < jobs.length ? jobs[i + 1].at : jobsBlock.length
		);
...
check(
	'every publishing job is guarded to the upstream repository',
	unguarded.length === 0,
	unguarded.join(', ')
);
...
check(
	'at least the three inherited publishers are guarded, so this is not vacuous',
	guarded.length >= 3,
	guarded.join(', ')
);
...
for (const file of files) {
	...
	for (let i = 0; i < jobs.length; i++) {
		...
		for (const [key, n] of seen) {
			if (n > 1)
				duplicates.push(file + ' job "' + jobs[i][1] + '" declares ' + key + ' ' + n + ' times');
		}
	}
}
check(
	'no job declares the same key twice, which YAML resolves by silently discarding one',
	duplicates.length === 0,
	duplicates.join(', ')
);
...
check(
	'a job that already had a condition kept it, ANDed with the guard',
	/github\.repository == 'open-webui\/open-webui' && \(!cancelled\(\)\)/.test(docker),
	'the guard must narrow a condition, never replace it'
);
check(
	'the branch and tag conditions survived too',
	/refs\/heads\/main/.test(docker) &&
		/refs\/tags\/v/.test(docker) &&
		/refs\/heads\/dev/.test(docker)
);

console.log('');
if (failures) {
	console.error(
		failures + " check(s) failed. A fork publishing upstream's package is impersonating it."
	);
	process.exit(1);
}
console.log(
	"Every inherited publishing job is fenced to upstream, and this project's own gates are not."
);
process.exit(0);
