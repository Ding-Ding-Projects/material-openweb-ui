...
check(
	'sequence numbers run from one, without gaps',
	log.entries.map((e) => e.seq).join(',') === '1,2,3,4'
);
check(
	'every entry carries a hash',
	log.entries.every((e) => /^[0-9a-f]{64}$/.test(e.hash))
);
check('the first entry has no parent', log.entries[0].parent === '');
check(
	'each later entry carries the hash of the one before it',
...
check('recording does not mutate the log it was given', JSON.stringify(log.entries) === before);

check(
	'an entry with no action is refused',
...
const tampered = {
	...log,
	entries: log.entries.map((e, i) => (i === 1 ? { ...e, after: 'English' } : e))
};
...
check(
	'the reason says the entry was changed, not merely that something is wrong',
	/has been changed/.test(verdict.reason),
	verdict.reason
);

const removed = { ...log, entries: [log.entries[0], log.entries[2], log.entries[3]] };
check('an entry removed from the middle is detected', (await H.verify(removed)).ok === false);

const reordered = {
	...log,
	entries: [log.entries[1], log.entries[0], log.entries[2], log.entries[3]]
};
check('entries put in the wrong order are detected', (await H.verify(reordered)).ok === false);

check(
	'the same content always hashes the same, whatever order the keys were built in',
	H.canonical({
		seq: 1,
		at: 'x',
		action: 'a',
...
check(
	'a restore appends rather than rolling the log back',
	restored.log.entries.length === log.entries.length + 1,
	String(restored.log.entries.length)
);
check(
	'the entry being restored from is still there, untouched',
	JSON.stringify(restored.log.entries[0]) === JSON.stringify(log.entries[0])
);
check(
	'the restore returns the value it is putting back',
	restored.restored === 'system',
	JSON.stringify(restored.restored)
);
check(
	'the new entry records the restore as what happened',
	restored.log.entries[4].action === 'restore'
);
check(
	'and it says which entry it came from',
	/theme/.test(restored.log.entries[4].target),
	restored.log.entries[4].target
);
check('the chain still verifies after a restore', (await H.verify(restored.log)).ok === true);
check(
	'restoring an entry that does not exist is refused',
...
check(
	'a label appends rather than editing the entry it describes',
	labelled.log.entries.length === log.entries.length + 1
);
check(
	'the entry it describes is byte-for-byte unchanged',
...
check(
	'the label in force is the one that was applied',
	H.labelFor(labelled.log, 2) === 'Before the trip',
	H.labelFor(labelled.log, 2)
);

const relabelled = await H.label(labelled.log, 2, 'Actually, after the trip', { at: T(8) });
check(
	'a second label supersedes the first without removing it',
	H.labelFor(relabelled.log, 2) === 'Actually, after the trip' &&
		relabelled.log.entries.filter((e) => e.action === 'label').length === 2
);
check(
	'a label longer than the limit is trimmed rather than refused',
	(await H.label(log, 1, 'x'.repeat(500))).log.entries.slice(-1)[0].after.length ===
		H.LIMITS.maxLabelLength
);
...
check(
	'a one-time-code secret is not written into the history',
	!written.includes('JBSWY3DPEHPK3PXP'),
	written.slice(0, 160)
);
check(
	'the field is present and marked, rather than dropped without trace',
...
check(
	'a secret nested deeper is caught too',
	(() => {
...
check(
	'an enormous value is truncated and says how much was left out',
	(() => {
...
check(
	'the action filter is derived from what is actually recorded',
	acts
		.map((a) => a.action)
		.sort()
		.join(',') === 'setting,tab',
...
check(
	'each action carries its real count',
	acts.find((a) => a.action === 'setting').count === 3,
	JSON.stringify(acts)
);
check('an action nobody has performed is not offered', !acts.some((a) => a.action === 'restore'));
check(
	'once one is performed, it appears',
...
const d = H.diff(
	{ theme: 'dark', language: 'English', keep: 1 },
	{ theme: 'light', funny: 3, keep: 1 }
);
check(
	'a changed field is reported',
	d.some((x) => x.key === 'theme' && x.kind === 'changed')
);
check(
	'a removed field is reported',
	d.some((x) => x.key === 'language' && x.kind === 'removed')
);
check(
	'an added field is reported',
	d.some((x) => x.key === 'funny' && x.kind === 'added')
);
check(
	'an unchanged field is NOT reported, which is what makes a diff readable',
	!d.some((x) => x.key === 'keep'),
	JSON.stringify(d.map((x) => x.key))
);
...
check(
	'the prune itself is recorded, so the log is honestly incomplete rather than quietly short',
	pruned.log.entries.some((e) => e.action === 'prune')
);
check(
	'the record says how many went',
	/2 entries removed/.test(pruned.log.entries.slice(-1)[0].label),
	pruned.log.entries.slice(-1)[0].label
);
...
const prunedKeeping = await H.prune(withLabel.log, {
	before: '2026-08-20',
	keepLabelled: true,
	at: T(9)
});
check(
	'a labelled entry survives a prune that would otherwise take it',
	prunedKeeping.log.entries.some((e) => e.seq === 1),
	prunedKeeping.log.entries.map((e) => e.seq).join(',')
);

const prunedAll = await H.prune(withLabel.log, {
	before: '2026-08-20',
	keepLabelled: false,
	at: T(9)
});
check(
	'and it does not survive one that says it is including labelled entries',
...
check(
	'the export produces one row per entry',
	exported.rows.length === secretish.log.entries.length
);
check(
	'the export carries no secret, because the history never had one',
	!JSON.stringify(exported.rows).includes('JBSWY3DPEHPK3PXP')
);
check(
	'the omission is stated rather than left to be noticed',
	exported.omitted.length > 0 && /not recorded/.test(exported.omitted[0]),
	JSON.stringify(exported.omitted)
);
check(
	'the omission names the fields it applies to',
	H.NEVER_RECORDED.every((f) => exported.omitted[0].includes(f)),
	exported.omitted[0]
);
check(
	'each row carries its hash, so an export can be checked against the log',
...
if (failures) {
	console.error(
		failures + ' check(s) failed. A history that can be quietly rewritten is not a record.'
	);
	process.exit(1);
}
console.log(
	'History only ever grows, the chain detects an edit, and no secret is ever written into it.'
);
process.exit(0);
