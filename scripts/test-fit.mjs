...
check(
	'no declared size gives Unknown rather than a guess',
	noSize.verdict === 'Unknown',
	noSize.verdict
);
check(
	'and it says why, rather than leaving a bare Unknown',
	noSize.evidence.some((e) => /did not report a size/.test(e)),
	JSON.stringify(noSize.evidence)
);
check(
	'the refusal to infer a size from the name is stated',
	noSize.assumptions.some((a) => /never inferred/.test(a)),
	JSON.stringify(noSize.assumptions)
);

const noMemory = fit(machine({ ram: null, vram: null }), { blobBytes: 2 * GB });
check(
	'a machine whose memory could not be measured gives Unknown',
	noMemory.verdict === 'Unknown',
	noMemory.verdict
);

const noRoom = fit(machine({ disk: 1 }), { blobBytes: 20 * GB });
check(
	'a model larger than the free space is Unlikely whatever the memory says',
	noRoom.verdict === 'Unlikely',
	noRoom.verdict
);
check(
	'and the disk is named as the reason',
	noRoom.evidence.some((e) => /Free space/.test(e)),
	JSON.stringify(noRoom.evidence)
);
...
check(
	'a longer declared context needs more room than a short one at identical weights',
	JSON.stringify(long) !== JSON.stringify(short),
	short.verdict + ' vs ' + long.verdict
);
check(
	'and the context window appears in the evidence',
	long.evidence.some((e) => /context window/i.test(e)),
	JSON.stringify(long.evidence)
);
check(
	'the allowance says it is a rule of thumb rather than a measurement',
	long.assumptions.some((a) => /rule of thumb/.test(a)),
	JSON.stringify(long.assumptions)
);

const declared = fit(machine(), {
	blobBytes: 4 * GB,
	parameterCount: '7B',
	quantisation: 'Q4_K_M',
	contextTokens: 8192
...
check(
	'a declared parameter count is reported',
	declared.evidence.some((e) => /Parameters: 7B/.test(e))
);
check(
	'a declared quantisation is reported',
	declared.evidence.some((e) => /Quantisation: Q4_K_M/.test(e))
);

const undeclared = fit(machine(), { blobBytes: 4 * GB });
check(
	'an undeclared parameter count is said to be undeclared, not silently ignored',
	undeclared.assumptions.some((a) => /parameter count was not declared/.test(a))
);
check(
	'an undeclared quantisation likewise',
	undeclared.assumptions.some((a) => /quantisation was not declared/.test(a))
);
check(
	'an undeclared context window makes the answer MORE conservative, never less',
...
for (const size of [1, 2, 4, 6, 8, 10, 14, 20]) {
	const withContext = fit(machine({ vram: 8, ram: 16 }), {
		blobBytes: size * GB,
		contextTokens: 4096
	});
	...
	if (RANK[without.verdict] > RANK[withContext.verdict]) {
		optimistic.push(
			size + 'GB: ' + without.verdict + ' without vs ' + withContext.verdict + ' with'
		);
	}
}
check(
	'missing metadata never produces a rosier verdict than declared metadata',
	optimistic.length === 0,
	optimistic.join(', ')
);
...
for (const [label, result] of [
	...
	check(
		'a "' + label + '" verdict carries the time it was measured',
		/^\d{4}-\d{2}-\d{2}T/.test(result.probedAt),
		result.probedAt
	);
}
check(
	'every verdict is one of the four the contract names',
...
check(
	'the main process imports fit',
	/import \{[^}]*\bfit\b[^}]*\} from '\.\/hardware\.js'/.test(main)
);
...
check(
	'the renderer has a bridge for it',
	/hardware:fit/.test(bridge) && /export const fitModels/.test(bridge)
);
check('the page calls that bridge', /desktop\s*\.\s*fitModels\(/.test(pageFlat) || /\.fitModels\(/.test(pageFlat));
...
check(
	'a missing verdict is stated rather than left blank',
	/Fit not measured yet|needs the desktop shell/.test(page),
	'an absent verdict and a verdict of Unknown are different facts'
);

check(
	'the verdicts recompute when the measured hardware changes',
	/onHardwareChanged\(/.test(page) && /hardwareListeners/.test(page)
);
check(
	'there is a control that changes the destination, or the recompute can never be triggered',
	/modelDestination/.test(page) && /model-destination/.test(page)
);
check(
	'changing it re-probes rather than reusing the old measurement',
...
check(
	'the hardware is probed once per call rather than once per model',
	/const hw = await probe\(/.test(main) &&
		/models\.map\(\(m\) => \(\{ id: String\(m\.id\), \.\.\.fit\(hw, m\)/.test(main)
);

console.log('');
if (failures) {
	console.error(
		failures + ' check(s) failed. A verdict nobody can reach is the same as no verdict.'
	);
	process.exit(1);
}
console.log(
	'Every verdict is evidenced, conservative when metadata is missing, and reaches the screen.'
);
process.exit(0);
