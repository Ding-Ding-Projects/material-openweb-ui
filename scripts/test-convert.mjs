...
check('unrecognised binary is reported as unrecognised', sniffed.type === 'Unrecognised binary', sniffed.type);
check('unrecognised binary shows its header as evidence', /Header: /.test(sniffed.evidence));

check('an empty file is reported, not crashed on', mod.sniff(new Uint8Array(0)).type === 'Empty file');
check('every result carries its evidence', ['{"a":1}', '%PDF-', 'hello'].every((s) => typeof mod.sniff(enc(s)).evidence === 'string'));
...
check('every adapter declares a category the registry knows', mod.ADAPTERS.every((a) => mod.CATEGORIES.includes(a.cat)));
check('every unavailable adapter names its exact missing dependency',
  mod.ADAPTERS.filter((a) => !a.available).every((a) => typeof a.reason === 'string' && a.reason.length > 30));
check('every available adapter discloses what a conversion changes',
  mod.ADAPTERS.filter((a) => a.available).every((a) => typeof a.discloses === 'string' && a.discloses.length > 10));
check('unavailable adapters are still listed rather than hidden',
  mod.ADAPTERS.some((a) => !a.available));
check('every category has at least one adapter listed',
  mod.CATEGORIES.every((c) => mod.ADAPTERS.some((a) => a.cat === c)));
check('a wildcard adapter matches any type', mod.adaptersFor('application/octet-stream').some((a) => a.id === 'to-hex'));
...
const csv = await mod.run(mod.ADAPTERS.find((a) => a.id === 'json-csv'), blobOf('[{"a":1,"b":"x"},{"a":2,"c":"y"}]'));
const csvText = await textOf(csv);
check('JSON to CSV unions every row\'s keys', csvText.split('\n')[0] === 'a,b,c', csvText.split('\n')[0]);
check('JSON to CSV leaves missing cells empty', csvText.includes('2,,y'), JSON.stringify(csvText));

const back = await mod.run(mod.ADAPTERS.find((a) => a.id === 'csv-json'), blobOf('a,b\n1,"x,y"\n'));
...
const pretty = await mod.run(mod.ADAPTERS.find((a) => a.id === 'json-pretty'), blobOf('{"b":1,"a":2}'));
check('pretty JSON preserves key order', (await textOf(pretty)).indexOf('"b"') < (await textOf(pretty)).indexOf('"a"'));

const lf = await mod.run(mod.ADAPTERS.find((a) => a.id === 'text-lf'), blobOf('a\r\nb\rc\n'));
check('line endings normalise to LF', (await textOf(lf)) === 'a\nb\nc\n', JSON.stringify(await textOf(lf)));

// base64 round trip through both adapters
const source = 'The quick brown fox jumps over the lazy dog.';
const b64 = await mod.run(mod.ADAPTERS.find((a) => a.id === 'to-b64'), blobOf(source));
const b64Text = await textOf(b64);
check('base64 encodes', b64Text === Buffer.from(source).toString('base64'), b64Text);
const roundTripped = await mod.run(mod.ADAPTERS.find((a) => a.id === 'from-b64'), blobOf(b64Text));
...
let rejected = false;
try {
  await mod.run(mod.ADAPTERS.find((a) => a.id === 'from-b64'), blobOf('not base64 !!!'));
} catch (e) {
...
const hex = await mod.run(mod.ADAPTERS.find((a) => a.id === 'to-hex'), blobOf('AB'));
const hexLine = (await textOf(hex)).split('\n')[0];
check('the hex dump line starts with an 8-digit offset', /^00000000 {2}/.test(hexLine), JSON.stringify(hexLine));
check('the hex dump line carries the bytes in hex', /\b41 42\b/.test(hexLine), JSON.stringify(hexLine));
check('the hex dump line ends with a delimited ascii pane', /\|AB\|$/.test(hexLine), JSON.stringify(hexLine));
check('the hex column is padded to a fixed width', hexLine.indexOf('|') === 59, String(hexLine.indexOf('|')));

// An unavailable adapter must refuse, not silently no-op.
let refused = false;
try {
  await mod.run(mod.ADAPTERS.find((a) => a.id === 'audio-wav'), blobOf('x'));
} catch (e) {
