...
const c = await import(pathToFileURL(join(process.cwd(), 'docs', 'assets', 'js', 'colour.js')).href);
...
check('sRGB white is L* 100 in D50 CIELAB', near(c.rgbToLab(white)[0], 100, 0.02), show(c.rgbToLab(white)));
check('sRGB white is neutral in CIELAB (a* and b* at zero)',
  near(c.rgbToLab(white)[1], 0, 0.02) && near(c.rgbToLab(white)[2], 0, 0.02), show(c.rgbToLab(white)));
check('sRGB black is L* 0 in CIELAB', near(c.rgbToLab(black)[0], 0, 0.02));
check('sRGB white is 1 in OKLab', near(c.rgbToOklab(white)[0], 1, 0.0005), show(c.rgbToOklab(white)));
check('sRGB white is neutral in OKLab',
  near(c.rgbToOklab(white)[1], 0, 0.0005) && near(c.rgbToOklab(white)[2], 0, 0.0005), show(c.rgbToOklab(white)));
...
const red = [1, 0, 0];
check('sRGB red is lab(54.29% 80.8 69.89)', nearAll(c.rgbToLab(red), [54.29, 80.8, 69.89], 0.06), show(c.rgbToLab(red)));
check('sRGB red is oklch(0.628 0.2577 29.23)', nearAll(c.rgbToOklch(red), [0.628, 0.2577, 29.23], 0.006), show(c.rgbToOklch(red)));

// Contrast has exact endpoints and one very well-known midpoint.
check('black on white is exactly 21 to 1', near(c.contrast(black, white), 21, 1e-9), String(c.contrast(black, white)));
check('a colour against itself is 1 to 1', near(c.contrast(red, red), 1, 1e-12));
check('#767676 on white is the classic 4.54 to 1',
  near(c.contrast(c.parse('#767676').rgb, white), 4.54, 0.01), String(c.contrast(c.parse('#767676').rgb, white).toFixed(3)));
check('#767676 on white grades AA but not AAA', c.grade(c.contrast(c.parse('#767676').rgb, white)) === 'AA');
check('a 3.2 to 1 ratio is honest about being large-text only',
...
for (const [name, forward, back, tol] of PAIRS) {
  ...
  for (const rgb of SAMPLES) {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (d > worst) { worst = d; worstAt = rgb; }
    }
  }
  worstAnywhere = Math.max(worstAnywhere, worst);
  check('every sample survives a trip through ' + name + ' (within ' + tol.toExponential(0) + ')',
    worst < tol, 'worst drift ' + worst.toExponential(2) + ' at ' + (worstAt ? show(worstAt) : ''));
}

// The bound that actually matters: no round trip may shift a colour by as much
// as the display can show.
check('no round trip drifts by even half an eight-bit step',
  worstAnywhere < 1 / 510, 'worst ' + worstAnywhere.toExponential(2) + ' vs ' + (1 / 510).toExponential(2));
...
for (const space of ['hex', 'rgb', 'hsl', 'hsv', 'hwb', 'lab', 'lch', 'oklab', 'oklch']) {
  ...
  let got = null;
  let err = null;
  try { got = c.parse(text).rgb; } catch (e) { err = e.message; }
  check(space + ' survives being written out and read back', got && nearAll(got, seed, 0.004),
...
check('cmyk survives being written out and read back',
  nearAll(c.parse(c.formats(seed).cmyk).rgb, seed, 0.004), c.formats(seed).cmyk);
...
check('a four-digit hex carries alpha', near(c.parse('#f008').alpha, 0x88 / 255, 1e-12), String(c.parse('#f008').alpha));
check('an eight-digit hex carries alpha', near(c.parse('#11223344').alpha, 0x44 / 255, 1e-12));
check('rgb() in 0-255 is read as 0-255', nearAll(c.parse('rgb(255 128 0)').rgb, [1, 128 / 255, 0], 1e-12));
check('rgb() in percentages is read as percentages', nearAll(c.parse('rgb(100% 50% 0%)').rgb, [1, 0.5, 0], 1e-12));
check('a comma-separated rgba() still parses', nearAll(c.parse('rgba(255, 0, 0, 0.5)').rgb, [1, 0, 0], 1e-12));
...
check('a named colour resolves', nearAll(c.parse('rebeccapurple').rgb, c.parse('#663399').rgb, 1e-12));
check('transparent is black at zero alpha',
  c.parse('transparent').alpha === 0 && nearAll(c.parse('transparent').rgb, [0, 0, 0], 1e-12));
check('every named colour parses', Object.keys(c.NAMED).every((n) => {
  try { c.parse(n); return true; } catch { return false; }
}));
check('the named set is the full CSS list, not a sample',
  Object.keys(c.NAMED).length === 148, String(Object.keys(c.NAMED).length) + ' names');

check('the space a value was WRITTEN in is remembered', c.parse('oklch(0.7 0.1 200)').space === 'oklch');
check('a hex with alpha is remembered as hex8', c.parse('#11223344').space === 'hex8');
check('a name is remembered as a name', c.parse('tomato').space === 'named' && c.parse('tomato').name === 'tomato');
...
check('the overshoot is measured rather than merely flagged', c.overshoot(wild) > 0.05, String(c.overshoot(wild)));
check('an ordinary colour is inside', c.inGamut(seed));
check('a round trip does not drift itself out of gamut',
  SAMPLES.every((s) => c.inGamut(c.oklchToRgb(c.rgbToOklch(s)))));

const described = c.describe('oklch(0.9 0.4 130)');
check('describe() warns before it clips rather than after',
  described.inGamut === false && c.inGamut(described.displayed),
  'displayed ' + show(described.displayed));
check('describe() reports contrast and a grade',
...
check('a colour that merely resembles a named one is not given its name',
  c.formats([1, 107 / 255, 71 / 255]).named === undefined, String(c.formats([1, 107 / 255, 71 / 255]).hex));
check('a colour with alpha is never given a name it cannot carry',
...
check('an empty string is refused', throws(() => c.parse('')));
check('a five-digit hex is refused', throws(() => c.parse('#12345')));
check('a hex with a non-hex digit is refused', throws(() => c.parse('#12345g')));
check('an unknown notation is refused', throws(() => c.parse('cielab(1 2 3)')));
check('a notation missing components is refused', throws(() => c.parse('rgb(255 0)')));
check('a bare word that is not a colour is refused', throws(() => c.parse('mauveish')));

console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. Every colour through this translator would be quietly wrong.');
  process.exit(1);
}
console.log('The translator matches published values and round-trips through every space it offers.');
process.exit(0);
