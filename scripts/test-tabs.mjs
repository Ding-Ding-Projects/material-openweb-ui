...
const LABELS = {
  chat: 'Chat', ollama: 'Ollama', converter: 'Converter',
  authenticator: 'Authenticator', settings: 'Settings', status: 'Status Hub', locks: 'Locks'
};
...
check('an empty model becomes a usable one rather than an empty strip',
  T.normalise({}, PAGES).tabs.length === 1);
check('a model with no tabs at all is repaired, not rendered empty',
  T.normalise({ tabs: [] }, PAGES).tabs.length === 1);
check('a tab pointing at a page this build does not have is dropped',
  T.normalise({ tabs: [{ id: 'a', page: 'chat' }, { id: 'b', page: 'a-page-from-the-future' }] }, PAGES).tabs.length === 1);
check('a tab in a group that no longer exists becomes ungrouped, not deleted', (() => {
  const m = T.normalise({ tabs: [{ id: 'a', page: 'chat', group: 'gone' }], groups: [] }, PAGES);
  return m.tabs.length === 1 && m.tabs[0].group === null;
})());
check('duplicate identifiers are made unique, since selection depends on them', (() => {
  const m = T.normalise({ tabs: [{ id: 'same', page: 'chat' }, { id: 'same', page: 'ollama' }] }, PAGES);
  return new Set(m.tabs.map((t) => t.id)).size === 2;
})());
check('an active tab that does not exist falls back to a real one', (() => {
...
check('an empty model opens on the page the caller names',
  T.normalise({}, PAGES, 'converter').tabs[0].page === 'converter',
  T.normalise({}, PAGES, 'converter').tabs[0].page);
check('a named landing page this build does not have falls back rather than breaking',
  PAGES.includes(T.normalise({}, PAGES, 'a-page-from-the-future').tabs[0].page));
check('a repaired empty model keeps the dock edge that was stored',
...
check('closing the active tab selects its NEIGHBOUR, not the first tab',
  closed.model.activeTab === 't4', closed.model.activeTab);
check('closing a tab removes exactly one', closed.model.tabs.length === 5);

const last = T.close({ ...m, activeTab: 't5' }, 't5');
check('closing the last tab in the strip falls back to the one before it',
  last.model.activeTab === 't4', last.model.activeTab);

const inactive = T.close(m, 't0');
check('closing a tab that is not active leaves the selection alone',
...
check('the other tabs keep their relative order',
  p.tabs.slice(1).map((t) => t.id).join(',') === 't0,t1,t3', ids(p));
p = T.setPinned(p, 't0', true).model;
check('a second pinned tab joins the pinned region rather than the general order',
  p.tabs[0].id === 't2' && p.tabs[1].id === 't0', ids(p));
...
check('unpinning leaves the pinned region', unpinned.tabs[0].id !== 't2' && !unpinned.tabs.find((t) => t.id === 't2').pinned,
  ids(unpinned));
check('and it lands at the front of the general region rather than at the end',
...
check('a pinned tab comes before every grouped tab, not only the ungrouped ones',
  withGroup.tabs[0].id === 't3' && withGroup.tabs[0].pinned === true,
  withGroup.tabs.map((t) => t.id + (t.pinned ? '*' : t.group ? '#' : '')).join(','));
check('and the groups still come before the ungrouped tabs', (() => {
...
const expectedO = b.tabs.filter((t) => (labelFor(t.page) + ' ' + t.page).toLowerCase().includes('o'));
check('the search finds exactly the tabs whose text contains the term',
  searched.tabs.length === expectedO.length &&
  searched.tabs.every((t) => expectedO.some((e) => e.id === t.id)),
  'found ' + searched.tabs.map((t) => t.page).join(',') + ' expected ' + expectedO.map((t) => t.page).join(','));
check('the term really does exclude some tabs, so the check is not vacuous',
  expectedO.length > 0 && expectedO.length < b.tabs.length, String(expectedO.length) + ' of ' + b.tabs.length);
check('the bulk close leaves the pinned tab alone even though it matched',
  bulk.model.tabs.some((t) => t.id === 't1'), pages(bulk.model));
check('the bulk close and the search share one notion of what a tab is', (() => {
...
const inverted = T.closeMatching(b, (s) => s.toLowerCase().includes('chat'), labelFor, { invert: true });
check('closing tabs NOT containing a term keeps the ones that do',
  inverted.model.tabs.some((t) => t.page === 'chat'), pages(inverted.model));
check('and it still spares the pinned tab that did not match',
  inverted.model.tabs.some((t) => t.id === 't1'), pages(inverted.model));

const everything = T.closeMatching(build(['chat', 'ollama']), () => true, labelFor);
check('a bulk close that would empty the strip is refused as a whole',
  everything.model.tabs.length === 2 && Boolean(everything.refused), everything.refused);
check('and it says how many it would have closed',
  everything.wouldClose === 2, String(everything.wouldClose));

const others = T.closeOthers(b, 't3');
check('close-others keeps the pinned tabs too, not only the chosen one',
  others.model.tabs.length === 2 && others.model.tabs.some((t) => t.id === 't1'), pages(others.model));
...
check('search 1, the current strip, finds tabs anywhere in it',
  T.searchStrip(s, term('conv'), labelFor).tabs.length === 1);
check('search 2, inside one group, finds only that group\'s tabs',
  T.searchInGroup(s, groupId, term('o'), labelFor).tabs.every((t) => t.group === groupId));
check('search 2 does not reach outside its group',
  T.searchInGroup(s, groupId, term('settings'), labelFor).tabs.length === 0);
check('search 3, groups by name, finds the group and not its tabs', (() => {
  const r = T.searchGroups(s, term('model'));
  return r.groups.length === 1 && r.tabs.length === 0;
})());
check('search 4, across everything, finds tabs AND groups together', (() => {
...
check('a tab is findable by its group name, not only by its own',
...
check('one function supplies the text every search reads',
  typeof T.haystack === 'function' &&
  T.haystack(s.tabs.find((t) => t.id === 't1'), labelFor, s.groups).includes('Model work'),
  T.haystack(s.tabs.find((t) => t.id === 't1'), labelFor, s.groups));
...
check('deleting a group does NOT delete its tabs',
  deleted.model.tabs.length === s.tabs.length, String(deleted.model.tabs.length) + ' vs ' + s.tabs.length);
check('the released tabs become ungrouped', deleted.model.tabs.every((t) => t.group !== groupId));
...
check('a group can be renamed', T.renameGroup(s, groupId, 'Renamed').model.groups.find((g) => g.id === groupId).name === 'Renamed');
check('a group name is bounded',
  T.renameGroup(s, groupId, 'x'.repeat(400)).model.groups.find((g) => g.id === groupId).name.length === T.LIMITS.maxNameLength);
check('a group colour must be one of the offered ones',
  T.setGroupColour(s, groupId, 'chartreuse').model.groups.find((g) => g.id === groupId).colour !== 'chartreuse');
check('a group can be collapsed', T.setGroupCollapsed(s, groupId, true).model.groups.find((g) => g.id === groupId).collapsed === true);
check('moving a tab into a group unpins it, since the regions are exclusive', (() => {
...
check('moving a tab to a group that does not exist changes nothing',
  T.moveToGroup(s, 't0', 'no-such-group').model.tabs.find((t) => t.id === 't0').group === null);

const reordered = T.reorderGroup(s, second.group.id, 0);
check('groups can be reordered', reordered.model.groups.find((g) => g.id === second.group.id).order === 0);
check('reordering a group reorders the tabs in the strip with it', (() => {
...
})(), T.reorderGroup(s, second.group.id, 0).model.tabs.map((t) => t.group || '-').join(','));
...
check('a tab can be moved along the strip',
  T.reorder(r, 't0', 2).model.tabs.map((t) => t.id).join(',') === 't1,t2,t0,t3',
  ids(T.reorder(r, 't0', 2).model));
check('a move past the end is clamped rather than losing the tab',
  T.reorder(r, 't0', 99).model.tabs.length === 4);
check('a pinned tab cannot be dragged out of the pinned region', (() => {
...
check('a tab inside a collapsed group is skipped rather than focused invisibly',
  !reachable(kc, 't0').includes('t1'), reachable(kc, 't0').join(','));
check('unless it is the active tab, which must stay reachable',
...
const big = { tabs: Array.from({ length: 500 }, (_, i) => ({ id: 'x' + i, page: 'chat' })), groups: [] };
check('a stored model beyond the tab limit is trimmed rather than loaded whole',
  T.normalise(big, PAGES).tabs.length === T.LIMITS.maxTabs, String(T.normalise(big, PAGES).tabs.length));
check('opening past the limit is refused with a reason', (() => {
...
console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. Every one of these looks like ordinary behaviour when it is wrong.');
  process.exit(1);
}
console.log('Pinned tabs survive bulk closes, groups outlive deletion, and all four searches read the same text.');
process.exit(0);
