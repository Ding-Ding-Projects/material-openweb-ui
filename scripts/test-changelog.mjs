...
const { CHANGELOG } = await import(pathToFileURL(join(ROOT, 'docs', 'assets', 'js', 'content.js')).href);
...
check('every release has a version', CHANGELOG.every((v) => typeof v.version === 'string' && v.version));
check('every release has a date in ISO form',
  CHANGELOG.every((v) => /^\d{4}-\d{2}-\d{2}$/.test(v.date)),
  CHANGELOG.filter((v) => !/^\d{4}-\d{2}-\d{2}$/.test(v.date)).map((v) => v.version).join(','));
check('releases are newest first, so the top of the page is the current one',
  CHANGELOG.every((v, i) => i === 0 || CHANGELOG[i - 1].date >= v.date),
  CHANGELOG.map((v) => v.version + '@' + v.date).join(' '));
check('every release has at least one section', CHANGELOG.every((v) => v.sections && v.sections.length));
check('every section has at least one item',
  CHANGELOG.every((v) => v.sections.every((s) => s.items && s.items.length)));
check('every item has text', CHANGELOG.every((v) => v.sections.every((s) => s.items.every((i) => i.text && i.text.length > 10))));
...
check('most entries name a commit',
...
for (const item of withoutSha) {
  check('an entry with no commit is a note rather than a change: "' + item.text.slice(0, 46) + '…"',
    /note/i.test(item.section || ''), item.section || 'no section recorded');
}
...
if (!repo) {
  ...
  check('this check ran somewhere it could resolve commits', false,
    'run it inside the repository');
} else {
  const bad = [];
  for (const item of withSha) {
    try {
      const type = execFileSync('git', ['cat-file', '-t', item.sha], { cwd: ROOT, encoding: 'utf8' }).trim();
      if (type !== 'commit') bad.push(item.sha + ' is a ' + type + ', not a commit');
    } catch {
      bad.push(item.sha + ' (' + item.version + ': ' + item.text.slice(0, 40) + '…) does not resolve');
    }
  }
  for (const b of bad) console.error('  FAIL  ' + b);
  failures += bad.length;
  check('every commit named by the changelog exists in this repository',
    bad.length === 0, String(withSha.length) + ' checked');
  ...
  for (const item of withSha) {
    try {
      execFileSync('git', ['merge-base', '--is-ancestor', item.sha, 'HEAD'], { cwd: ROOT, stdio: 'ignore' });
    } catch {
      unreachable.push(item.sha);
    }
  }
  check('every commit named is actually reachable from this branch',
    unreachable.length === 0, unreachable.join(', '));
}

console.log('');
if (failures) {
  console.error(failures + ' check(s) failed. A changelog nobody can verify is a changelog nobody should believe.');
  process.exit(1);
}
