...
const F = await import(
	pathToFileURL(join(process.cwd(), 'docs', 'assets', 'js', 'formats.js')).href
);
...
check(
	'every format in the list has a writer',
	F.FORMATS.every((f) => {
		try {
			F.serialise(SIMPLE, f.id);
			return true;
		} catch {
			return false;
		}
...
check(
	"the contract's nine formats are all present",
	['json', 'jsonl', 'yaml', 'toml', 'xml', 'csv', 'tsv', 'markdown', 'html'].every((id) =>
		F.FORMATS.some((f) => f.id === id)
	)
);
check(
	'an unknown format is refused rather than silently producing JSON',
	(() => {
		try {
			F.serialise(SIMPLE, 'parquet');
			return false;
		} catch {
			return true;
		}
...
check(
	'an empty list produces a valid document in every format',
...
check(
	'JSON round-trips every awkward value exactly',
...
check(
	'JSON Lines round-trips every awkward value exactly',
	JSON.stringify(
		F.toJsonLines(AWKWARD)
			.trim()
			.split(NL)
			.map((l) => JSON.parse(l))
...
check(
	'a newline inside a value does not become a record boundary',
...
for (const [name, write, delimiter] of [
	['CSV', F.toCsv, ','],
	['TSV', F.toTsv, '\t']
]) {
	...
	check(
		name + ' survives a field containing the delimiter',
		back[0] && back[0].comma === AWKWARD[0].comma,
		back[0] && JSON.stringify(back[0].comma)
	);
	check(
		name + ' survives a field containing a quote',
		back[0] && back[0].quote === AWKWARD[0].quote,
		back[0] && JSON.stringify(back[0].quote)
	);
	check(
		name + ' survives a field containing a line break',
		back[0] && back[0].newline === AWKWARD[0].newline,
		back[0] && JSON.stringify(back[0].newline)
	);
	check(
		name + ' keeps an empty field empty rather than dropping the column',
		back[1] && back[1].comma === '',
		back[1] && JSON.stringify(back[1].comma)
	);
	check(
		name + ' keeps every column',
...
check(
	'a field beginning with = is neutralised so a spreadsheet cannot run it',
	F.delimitedField('=SUM(A1:A9)', ',').startsWith("'"),
	F.delimitedField('=SUM(A1:A9)', ',')
);
...
check('CSV uses CRLF line endings, as RFC 4180 specifies', F.toCsv(SIMPLE).includes('\r\n'));
...
check(
	...
	xml.includes('&amp;') && !/&(?!amp;|lt;|gt;|quot;|apos;)/.test(xml)
);
check(
	'angle brackets in a value are escaped, so they cannot become markup',
	xml.includes('&lt;script&gt;') && !xml.includes('<script>')
);
check('every record is present', (xml.match(/<record>/g) || []).length === AWKWARD.length);
check('a value containing a newline survives', xml.includes('first' + NL + 'second'));
...
if (xmlParsed === null) {
	...
	check(
		'no unescaped entity start survives anywhere in the document (no parser available to confirm)',
...
check(
	'the word "no" is quoted, or it comes back as the boolean false',
	/plain: "no"/.test(yaml),
	yaml.split(NL).find((l) => l.includes('plain:'))
);
check(
	'a digit string with a leading zero is quoted, or it loses the zero',
	/newline: "007"/.test(yaml),
	yaml.split(NL).find((l) => l.includes('newline: 0'))
);
check(
	'a value starting with a dash is quoted, or it becomes a list item',
	/pipe: "- leading dash"/.test(yaml),
	yaml.split(NL).find((l) => l.includes('pipe:'))
);
check(
	'a value containing a newline is quoted with the break escaped',
	/newline: "first\\nsecond"/.test(yaml),
	yaml.split(NL).find((l) => l.includes('first'))
);
...
check(
	'each record is an array-of-tables entry',
	(toml.match(/^\[\[record\]\]$/gm) || []).length === AWKWARD.length
);
check('a quote inside a value is escaped', toml.includes('\\"hello\\"'));
check(
	'a newline inside a value is escaped rather than breaking the line',
	toml.includes('first\\nsecond'),
	toml.split(NL).find((l) => l.includes('first'))
);
...
check(
	'there is a header row and a separator row',
	mdLines.length === AWKWARD.length + 2,
	String(mdLines.length)
);
check(
...
check(
	'a pipe inside a value is escaped, or it silently shifts every column',
	md.includes('left \\| right'),
	mdLines.find((l) => l.includes('right'))
);
check(
	'a newline inside a value becomes a break rather than a new row',
...
check(
	'a script in a value cannot become a script in the page',
	html.includes('&lt;script&gt;') && !html.includes('<script>')
);
check(
	'there is a header cell per column',
...
check(
	'the column set is the union across every record, not just the first',
	F.columns([{ a: 1 }, { b: 2 }]).join(',') === 'a,b'
);
check('columns keep first-seen order', F.columns([{ z: 1, a: 2 }, { m: 3 }]).join(',') === 'z,a,m');
check(
	'a record missing a column exports an empty cell, not a shifted row',
	(() => {
...
check(
	'a filename with path separators is made safe',
...
if (failures) {
	console.error(
		failures + ' check(s) failed. Every one of these produces a file that opens without complaint.'
	);
	process.exit(1);
}
console.log(
	'Every format survives the values that break naive writers, and the delimited ones read back exactly.'
);
process.exit(0);
