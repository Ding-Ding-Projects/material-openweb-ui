...
function host() {
  if (!toastHost) {
    toastHost = h('div', { class: 'toasts', role: 'region', 'aria-label': 'Notifications', 'aria-live': 'polite' });
...
export function notify(message, opts = {}) {
  ...
  const body = h('div', { class: 'toast__body' },
    opts.title ? h('div', { class: 'toast__title' }, opts.title) : null,
    h('div', {}, message)
  );

  const el = h('div', { class: 'toast' + (kind === 'error' ? ' toast--err' : kind === 'ok' ? ' toast--ok' : ''), role: kind === 'error' ? 'alert' : 'status' },
    icon(kind === 'error' ? 'warn' : kind === 'ok' ? 'check' : 'info', 'icon icon--sm'),
    body,
    opts.action ? h('button', { type: 'button', class: 'toast__act', onclick: () => { opts.action.run(); close(); } }, opts.action.label) : null,
    h('button', {
      type: 'button', class: 'btn btn--icon btn--text', 'aria-label': i18n.t('action.close'),
      style: { width: '28px', height: '28px', minWidth: '28px' }, onclick: () => close()
    }, icon('x', 'icon icon--sm'))
  );
...
export function dismissNotifications(ids) {
  ...
  store.set('notifications', next, { action: 'deleted', label: ids.length + ' notification(s) dismissed' });
  return next;
}
  ...
  const el = h('div', {
    ...
    h('div', { class: 'dialog__actions' },
      ...actions.map((a) => h('button', {
        type: 'button',
        class: 'btn ' + (a.primary ? 'btn--filled' : a.danger ? 'btn--danger' : 'btn--outlined'),
        onclick: () => { const keep = a.run && a.run(); if (!keep) close(); }
      }, a.label))
    )
  );
...
export function menu(anchor, items, opts = {}) {
  const list = h('div', { role: 'menu', style: { display: 'flex', flexDirection: 'column', gap: '2px' } });
  ...
  function render() {
    ...
    if (!real.length) {
      list.appendChild(h('div', { class: 'menu__empty' }, m.ok ? i18n.t('empty.noMatch') : 'That pattern is not valid yet.'));
    } else {
      for (const it of shown) {
        if (it.separator) { list.appendChild(h('hr', { style: { margin: '4px 0' } })); continue; }
        list.appendChild(h('button', {
          type: 'button', class: 'menu__item', role: 'menuitem',
          dataset: it.danger ? { danger: 'true' } : {},
          onclick: () => { handle.close(); it.run && it.run(); }
        },
    ...
    count.textContent = real.length + ' of ' + items.filter((i) => !i.separator).length + ' items shown';
    handle && handle.reposition && handle.reposition();
  }

  field.onChange(render);

  const body = h('div', { class: 'menu', style: { display: 'flex', flexDirection: 'column', gap: '8px' } },
    field.el, count, list
  );
  ...
  const handle = popover(anchor, body, { width: opts.width || 300, role: 'menu', label: opts.label || 'Menu' });
  render();
...
export function select({ value, options, onChange, label, width = 220 }) {
  const btn = h('button', {
    type: 'button', class: 'btn btn--outlined', 'aria-haspopup': 'listbox', 'aria-label': label,
  ...
  return {
    el: btn,
    set(v) { btn.firstChild.textContent = labelFor(v); }
  };
}
...
export function superConfirm({ what, affects, onConfirm }) {
  ...
  const phrase = h('input', {
    type: 'text', placeholder: 'DELETE', 'aria-label': i18n.t('confirm.type'), class: 'mono',
    style: { flex: '1', border: '0', background: 'transparent', outline: 'none' },
    oninput: (e) => { keyA = e.target.value.trim().toUpperCase() === 'DELETE'; sync(); }
  });

  const holdBtn = h('button', {
    type: 'button', class: 'btn btn--outlined', 'aria-pressed': 'false',
    onclick: () => { keyB = !keyB; holdBtn.setAttribute('aria-pressed', String(keyB)); sync(); }
  }, icon('lock', 'icon icon--sm'), i18n.t('confirm.hold'));

  const slider = h('input', {
    type: 'range', min: '0', max: '100', value: '0', class: 'slider', disabled: true,
    'aria-label': i18n.t('confirm.slide'),
    oninput: () => { fill.style.width = slider.value + '%'; },
    onchange: () => {
      if (Number(slider.value) >= 100 && keyA && keyB) {
        done.textContent = 'Confirmed.';
        setTimeout(() => { d.close(); onConfirm(); }, 260);
      } else if (Number(slider.value) < 100) {
        slider.value = '0'; fill.style.width = '0%';
      }
    }
  });

  const fill = h('div', { style: { height: '6px', width: '0%', background: 'var(--err)', borderRadius: '3px', transition: 'width .08s linear' } });
  const track = h('div', { style: { height: '6px', background: 'var(--schighest)', borderRadius: '3px', marginTop: '-13px', pointerEvents: 'none' } }, fill);
  const done = h('div', { style: { fontSize: '.78rem', color: 'var(--ok)', minHeight: '18px', fontWeight: '600' } });
  ...
  function sync() {
    ...
    gate.textContent = ready
      ...
      : 'Key 1: type DELETE — ' + (keyA ? 'set' : 'not set') + '. Key 2: hold — ' + (keyB ? 'set' : 'not set') + '.';
    if (!ready) { slider.value = '0'; fill.style.width = '0%'; }
  }
  ...
  const d = dialog({
    ...
    body: h('div', { class: 'stack', style: { gap: '16px' } },
      h('div', { class: 'notice' },
        icon('warn'),
        h('div', {},
          h('div', { style: { fontWeight: '700', marginBottom: '4px' } }, 'This cannot be undone from here.'),
