...
export function searchDialog({ scope, model, labelFor, groupId, onPick, onModel }) {
	...
	function paint() {
		...
		const found =
			scope === 'strip'
				? tabsCore.searchStrip(model, matcher.test, labelFor)
				: scope === 'group'
					? tabsCore.searchInGroup(model, groupId, matcher.test, labelFor)
					: scope === 'groups'
						? tabsCore.searchGroups(model, matcher.test)
						: tabsCore.searchEverything(model, matcher.test, labelFor);

		const total = found.tabs.length + found.groups.length;
		add(
			summary,
			h('span', {}, total === 0 ? 'Nothing matched.' : total + ' result' + (total === 1 ? '' : 's'))
		);
		...
		for (const g of found.groups) {
			add(
				...
				h(
					...
					{
						type: 'button',
						class: 'tabsearch__row tabsearch__row--group',
						onclick: () => {
							onModel(tabsCore.setGroupCollapsed(model, g.id, false).model, g.id);
						}
					},
			...
			scope === 'strip' || scope === 'everything'
				? h(
						...
						h(
							...
							h(
								'button',
								{ class: 'btn btn--outlined btn--sm', onclick: () => bulk(false) },
								'Close tabs that match'
							),
							h(
								'button',
								{ class: 'btn btn--outlined btn--sm', onclick: () => bulk(true) },
								'Close tabs that do not match'
							)
						),
						h(
							'p',
							{ class: 'tabsearch__note' },
	...
	function bulk(invert) {
		...
		ui.notify(result.closed + ' tab' + (result.closed === 1 ? '' : 's') + ' closed.', {
			kind: 'ok'
		});
		d.close();
...
function groupMenu(anchor, model, group, apply) {
	...
	ui.menu(
		...
		[
			{
				label: group.collapsed ? 'Expand this group' : 'Collapse this group',
				icon: 'grid',
			...
			{ separator: true },
			{
				label: 'Search inside this group…',
				icon: 'search',
				run: () =>
					searchDialog({
						scope: 'group',
	...
	ui.dialog({
		...
		actions: [
			{ label: 'Cancel' },
			{
				label: 'Rename',
				primary: true,
				run: () => apply(tabsCore.renameGroup(model, group.id, input.value).model)
			}
		]
	});
}
...
export function renderStrip(container, { model, apply, labelFor, iconFor, extras }) {
	...
	function tabButton(t) {
			...
			t.pinned
				...
				: h(
						'span',
						{
							class: 'wtab__close',
							role: 'button',
							'aria-label': 'Close ' + labelFor(t.page),
							onclick: (e) => {
								e.stopPropagation();
								apply(tabsCore.close(model, t.id).model);
							}
						},
						icon('x', 'icon icon--sm')
					)
		...
		btn.addEventListener('drop', (e) => {
			...
			const region = model.tabs.filter(
				(x) =>
					(x.pinned ? 'pinned' : x.group || 'loose') === (t.pinned ? 'pinned' : t.group || 'loose')
			);
	...
	function tabMenu(anchor, t) {
		const groupItems = [
			...
			})),
			{
				label: 'New group…',
				run: () => {
					const made = tabsCore.createGroup(
						model,
						'Group ' + (model.groups.length + 1),
						tabsCore.GROUP_COLOURS[model.groups.length % tabsCore.GROUP_COLOURS.length].id
					);
					if (made.error) {
						ui.notify(made.error, { kind: 'bad' });
						return;
					}
			...
			t.group
				? {
						label: 'Take it out of its group',
						run: () => apply(tabsCore.moveToGroup(model, t.id, null).model)
					}
				: null
		].filter(Boolean);
		...
		ui.menu(
			...
			[
				{
					label: t.pinned ? 'Unpin this tab' : 'Pin this tab',
					icon: 'lock',
				...
				{
					label: 'Close tabs containing text…',
					icon: 'search',
					run: () =>
						searchDialog({
							scope: 'strip',
		...
		if (group && group.id !== lastGroup) {
			...
			const header = h(
				'button',
				{
					...
					oncontextmenu: (e) => {
						e.preventDefault();
						groupMenu(header, model, group, apply);
					}
				},
				h('span', { class: 'tabgroup__dot' }),
				group.name,
				group.collapsed ? h('span', { class: 'tabgroup__count' }, String(count)) : null
			);
	...
	container.addEventListener('keydown', (e) => {
		...
		if (e.key === 'Home' || e.key === 'End') {
			...
			apply({
				...model,
				activeTab: (e.key === 'Home' ? visible[0] : visible[visible.length - 1]).id
			});
		}
	});
	...
	const stripMenu = (anchor) =>
		ui.menu(
			anchor,
			[
				{
					label: 'Dock the strip',
					icon: 'desktop',
					sub: tabsCore.DOCKS.map((d) => ({
