	...
	function show() {
		...
		const rxBtn = h(
			'button',
			{
				...
				title: 'Open the regex builder for this field',
				'aria-label': 'Open the regex builder for this field',
				onclick: () => openBuilder()
			},
			'.*'
		);
		...
		function render() {
			...
			if (!rows.length) {
				list.appendChild(
					h('div', { class: 'menu__empty' }, m.ok ? emptyText : 'That pattern is not valid yet.')
				);
			...
			rows.forEach((e, i) => {
				...
				if (e.control) {
					row.appendChild(
						h(
							'span',
							{ class: 'palette__control', onclick: (ev) => ev.stopPropagation() },
							e.control
						)
					);
					if (e.run) {
						row.appendChild(
							h(
								...
								{
									class: 'btn btn--text',
									style: { height: '30px', fontSize: '.72rem' },
									onclick: (ev) => {
										ev.stopPropagation();
										close();
										e.run();
		...
		const el = h(
			...
			h(
				...
				h(
					...
					{
						class: 'btn btn--text',
						style: { height: '32px', fontSize: '.72rem' },
						onclick: () => toggleSize(),
				...
				h(
					'span',
					{ class: 'chip chip--tonal mono', style: { height: '24px', fontSize: '.66rem' } },
					'esc'
				)
			),
			live,
