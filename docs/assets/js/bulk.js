		...
		append(
			...
			h(
				...
				h('input', {
					...
					ref: (el) => {
						el.indeterminate = sum.visible > 0 && !allSelected;
					},
					'aria-label': allSelected ? 'Deselect everything visible' : 'Select everything visible',
					onchange: () =>
						set(
							allSelected
								? sel.clearInScope(selection, scopeIds)
								: sel.selectAllInScope(selection, scopeIds)
						)
				}),
			...
			h(
				'button',
				{ class: 'bulk__link', onclick: () => set(sel.invertInScope(selection, scopeIds)) },
				'Invert'
			),
			sum.selected
				? h('button', { class: 'bulk__link', onclick: () => set(sel.clear()) }, 'Clear selection')
				: null,
			h(
				'span',
				{ class: 'bulk__count', role: 'status' },
			...
			...actions.map((a) =>
				h(
					...
					a.icon ? icon(a.icon, 'icon icon--sm') : null,
					a.label
				)
			),
			exportRows
			...
			ui.dialog({
				...
				body: h(
					...
					h(
						'p',
						{},
						sel.describeScope(sum, { verb: action.label.toLowerCase(), noun, nounPlural })
					),
					sum.hidden
						? h(
								'div',
					...
					sum.stale
						? h(
								...
								say(sum.stale) +
									' selected ' +
									(sum.stale === 1 ? 'no longer exists' : 'no longer exist') +
									' and will be skipped.'
							)
						: null
				),
	...
	function exportDialog() {
		...
		ui.dialog({
			...
			body: h(
				...
				h(
					'p',
					{ class: 'muted', style: { fontSize: '.8rem', lineHeight: '1.6' } },
			...
			actions: [
				{ label: 'Cancel' },
				{
					...
					run: () => {
						...
						ui.downloadFile(
							formats.filenameFor(sel.plural(2, noun, nounPlural).slice(2), format),
							formats.serialise(rows, format),
							f.mime
						);
					}
				}
