...
export function compile(pattern, flags) {
	...
	if (pattern.length > MAX_PATTERN)
		return { ok: false, error: 'Pattern is longer than ' + MAX_PATTERN + ' characters' };
	try {
...
export function searchField(opts = {}) {
	...
	const el = h(
		'div',
		{ class: 'field' + (opts.className ? ' ' + opts.className : '') },
	...
	function syncInput() {
		...
		input.placeholder = state.useRegex
			? 'pattern, e.g. ^ollama.*chat$'
			: opts.placeholder || i18n.t('search.site');
	...
	function openBuilder() {
		...
		const pat = h('input', {
			type: 'text',
			class: 'mono',
			value: state.pattern || state.query,
		...
		sample.value =
			state.sample || (opts.sampleFrom ? opts.sampleFrom().slice(0, 6).join('\n') : '');

		const flagI = h('input', {
			type: 'checkbox',
			checked: state.flags.includes('i'),
			style: { accentColor: 'var(--p)' },
			onchange: () => live()
		});
		const flagM = h('input', {
			type: 'checkbox',
			checked: state.flags.includes('m'),
			style: { accentColor: 'var(--p)' },
			onchange: () => live()
		});
		...
		const status = h('div', {
			class: 'mono',
			style: { fontSize: '.74rem', minHeight: '18px', lineHeight: '1.5' }
		});
		const groups = h('div', {
			style: { fontSize: '.74rem', color: 'var(--onsv)', lineHeight: '1.6' }
		});
		...
		function live() {
			...
			status.textContent =
				'✓ valid · ' + count + (count === 1 ? ' match' : ' matches') + ' in the sample';
			status.style.color = 'var(--ok)';
			groups.textContent = firstGroups
				? 'First match groups: ' +
					firstGroups.map((g, i) => '$' + (i + 1) + '=' + JSON.stringify(g ?? null)).join('  ')
				: '';
		}
		...
		function insert(tok) {
			...
			const inner =
				tok.includes('…') && e > s ? tok.replace('…', pat.value.slice(s, e)) : tok.replace('…', '');
		...
		const body = h(
			...
			h(
				'div',
				{ style: { display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '.78rem' } },
				h(
					'label',
			...
			h(
				'div',
				{ style: { fontSize: '.7rem', color: 'var(--onsv)', lineHeight: '1.55' } },
				i18n.t('rx.engine')
			),
			h(
				'div',
				...
				h(
					'button',
					{
						...
						class: 'btn btn--text',
						onclick: () => {
							navigator.clipboard?.writeText(pat.value);
						}
					},
					i18n.t('action.copy')
				),
