...
export const ROWS = [
	{
		...
		set: (v) =>
			store.patchSettings({ language: v }, { path: 'language', label: 'Language mode → ' + v }),
	...
	},
	{
		...
		set: (v) =>
			store.patchSettings(
				{ funnyEn: Number(v) },
				{ path: 'funnyEn', label: 'Funny level (English) → ' + v }
			),
		hiddenUnderSchool: true
	},
	{
		...
		set: (v) =>
			store.patchSettings(
				{ funnyZh: Number(v) },
				{ path: 'funnyZh', label: 'Funny level (Cantonese) → ' + v }
			),
		hiddenUnderSchool: true
	},
	{
	...
	},
	{
		...
		why: () =>
			'Scales every text size on this site proportionally. Layouts are checked at each step for clipping, so a larger size reflows rather than truncating.',
		kind: 'range',
		min: 0.85,
		max: 1.4,
		step: 0.05,
		...
		set: (v) =>
			store.patchSettings(
				{ scale: Number(v) },
				{ path: 'scale', label: 'Text size → ' + Math.round(v * 100) + '%' }
			)
	},
	{
		id: 'set-density',
		...
		why: () =>
			'Adds or removes height from controls without changing their type size, so a denser layout stays as readable and as tappable as a comfortable one.',
		kind: 'range',
		min: -1,
		max: 2,
		step: 1,
		...
		set: (v) =>
			store.patchSettings({ density: Number(v) }, { path: 'density', label: 'Density → ' + v })
	},
	{
		id: 'set-radius',
		path: 'radiusScale',
		...
		why: () =>
			'Scales every corner radius together. Material 3 shape is a system rather than a per-element choice, so this moves the whole product at once.',
		kind: 'range',
		min: 0,
		max: 1.5,
		step: 0.1,
		...
		set: (v) =>
			store.patchSettings(
				{ radiusScale: Number(v) },
				{ path: 'radiusScale', label: 'Corner radius → ' + v }
			)
	},
	{
		id: 'set-emoji',
		...
		set: (v) =>
			store.patchSettings(
				{ emojiDialogs: !!v },
				{ path: 'emojiDialogs', label: 'Emoji in dialogs → ' + (v ? 'on' : 'off') }
			),
		hiddenUnderSchool: true
	},
	{
		...
		set: (v) =>
			store.patchSettings(
				{ appName: v },
				{ path: 'appName', label: 'Display name → ' + (v || '(shipped name)') }
			)
	},
	{
		id: 'set-palette-size',
		...
		why: () =>
			'The palette opens as a bounded card by default and can be switched to a full-window view. The choice is remembered.',
		kind: 'select',
		options: () => [
			{ value: 'card', label: 'Bounded card' },
			{ value: 'full', label: 'Full window' }
		],
		get: () => store.settingValue('paletteSize'),
		set: (v) =>
			store.patchSettings({ paletteSize: v }, { path: 'paletteSize', label: 'Palette size → ' + v })
	}
];
...
export function control(row, onAfter) {
	...
	if (row.kind === 'range') {
		const out = h('span', {
			class: 'mono',
			style: { fontSize: '.74rem', color: 'var(--onsv)', minWidth: '108px', textAlign: 'right' }
		});
		const input = h('input', {
			type: 'range',
			class: 'slider',
			...
			style: { width: '150px' },
			oninput: (e) => {
				out.textContent = row.format ? row.format(Number(e.target.value)) : e.target.value;
			},
			onchange: (e) => {
	...
	if (row.kind === 'text') {
		const input = h('input', {
			...
			'aria-label': row.label(),
			onchange: (e) => {
				row.set(e.target.value.slice(0, 80));
				after();
			}
		});
		return {
...
export function renderRow(row, onAfter) {
	...
	const el = h(
		'div',
		{ class: 'setting', id: row.id, dataset: { settingRow: row.id } },
		h(
			'div',
			{ class: 'setting__main' },
			h('div', { class: 'setting__label' }, row.label()),
			i18n.isBilingual()
				? h('div', { class: 'muted cjk', style: { fontSize: '.8rem' } }, '')
				: null,
