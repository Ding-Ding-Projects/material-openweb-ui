...
export function rgbToHwb(rgb) {
  ...
  return [hue, Math.min(rgb[0], rgb[1], rgb[2]) * 100, (1 - Math.max(rgb[0], rgb[1], rgb[2])) * 100];
}
...
export function parse(input) {
  ...
  if (text === 'transparent') return { rgb: [0, 0, 0], alpha: 0, space: 'named', name: 'transparent' };
  ...
  switch (name) {
    ...
    case 'rgba': {
      ...
      return { rgb: v.map((c) => (percent ? c / 100 : c / 255)), alpha: alphaOf(body, 3), space: 'rgb' };
    }
    case 'hsl':
    case 'hsla':
...
export function overshoot(rgb) {
  return Math.max(0, Math.max.apply(null, rgb.map((c) => Math.max(-c, c - 1))));
}
