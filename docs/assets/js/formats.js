	...
	for (const row of rows) {
		...
		for (const [key, value] of Object.entries(row || {})) {
			...
			if (name) {
				...
				out.push(
					'    <field name="' + xmlEscape(key) + '">' + xmlEscape(scalar(value)) + '</field>'
				);
			}
		}
		out.push('  </' + item + '>');
...
export function parseDelimited(text, delimiter) {
	...
	return rows
		.slice(1)
		.map((r) => Object.fromEntries(header.map((h, j) => [h, r[j] === undefined ? '' : r[j]])));
}
...
function markdownCell(value) {
	return scalar(value).replace(/\\/g, '\\\\').replace(/\|/g, '\\|').replace(/\r?\n/g, '<br>');
}
...
export function toHtml(rows, { title = 'Export' } = {}) {
	...
	const body = rows
		.map((r) => '<tr>' + cols.map((c) => cell(r ? r[c] : '')).join('') + '</tr>')
		.join(NL);
