...
function emit(key, value) {
  for (const fn of listeners) {
    try { fn(key, value); } catch (e) { console.error('mowui: listener failed', e); }
  }
}
...
export const EXPORT_FORMATS = ['json', 'jsonl', 'yaml', 'toml', 'xml', 'csv', 'tsv', 'markdown', 'html'];
...
export function exportBundle() {
  const s = clone(state);
  s.locks = Object.fromEntries(Object.entries(s.locks || {}).map(([k, v]) => [k, { ...v, secret: undefined, hash: undefined }]));
  s.totp = (s.totp || []).map((e) => ({ ...e, secret: undefined }));
  if (s.settings) s.settings = { ...s.settings, vocab: s.settings.vocab ? { loaded: true, terms: '[omitted]' } : null };
  return {
