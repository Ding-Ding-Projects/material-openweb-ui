...
export function render(root) {
	...
	const warning = h(
		...
		h(
			...
			h(
				'p',
				{ class: 'dl__warntext dl__warntext--zh cjk' },
	...
	if (!RELEASES.length) {
		add(
			...
			h(
				...
				h(
					'span',
					{ class: 'muted', style: { fontSize: '.86rem', maxWidth: '60ch' } },
				...
				h(
					...
					...r.files.map((f) =>
						h(
							...
							h(
								'a',
								{ href: f.url, class: 'btn btn--filled', rel: 'noreferrer noopener' },
	...
	const verify = h(
		...
		h(
			...
			'# Windows' +
				String.fromCharCode(10) +
				'Get-FileHash .\\material-openweb-ui-setup.exe -Algorithm SHA256' +
				String.fromCharCode(10) +
				String.fromCharCode(10) +
				'# macOS and Linux' +
				String.fromCharCode(10) +
	...
	add(
		...
		h(
			...
			h(
				'p',
				{ class: 'muted' },
				'What there is to install, and what your operating system will say about it.'
			)
		),
		warning,
		list,
