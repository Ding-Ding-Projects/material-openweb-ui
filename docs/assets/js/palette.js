	...
	out.push(
		{
			...
			hint: 'action',
			run: () => window.mowui.toggleTheme()
		},
		{
			kind: 'action',
			id: 'act-export',
			icon: 'download',
			label: 'Export everything this site has stored',
			hint: 'action',
			run: () => window.mowui.exportAll()
		},
		{
			kind: 'action',
			id: 'act-size',
			icon: 'grid',
			label: 'Switch palette between card and full window',
			hint: 'action',
			run: () => palette.toggleSize()
		},
		{
			kind: 'action',
			id: 'act-reset',
			...
			hint: 'action · destructive',
			run: () => window.mowui.open('settings', null, 'set-theme')
		}
	);
