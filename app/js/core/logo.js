...
export function presetSvg(preset, size = 64) {
  return '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="' + size +
    '" height="' + size + '" role="img" aria-label="' + preset.label + ' application mark">' +
    preset.svg + '</svg>';
}
...
export function readDimensions(u8) {
  ...
  if (has(u8, 0, [0x89, 0x50, 0x4e, 0x47, 0x0d, 0x0a, 0x1a, 0x0a])) {
    if (u8.length < 24 || !has(u8, 12, [0x49, 0x48, 0x44, 0x52])) {
      throw new Error('The PNG signature is present but the IHDR header is not where a PNG puts it.');
    }
    ...
    for (let i = 8; i + 8 < head.length; i++) {
      if (has(head, i, [0x61, 0x63, 0x54, 0x4c])) { frames = be32(head, i + 4) || 2; break; }
    }
  ...
  if (has(u8, 0, [0x52, 0x49, 0x46, 0x46]) && has(u8, 8, [0x57, 0x45, 0x42, 0x50])) {
    if (has(u8, 12, [0x56, 0x50, 0x38, 0x58])) { // VP8X, extended
    ...
    if (has(u8, 12, [0x56, 0x50, 0x38, 0x20])) { // VP8, lossy
      return { width: le16(u8, 26) & 0x3fff, height: le16(u8, 28) & 0x3fff, frames: 1, format: 'WebP' };
    }
    if (has(u8, 12, [0x56, 0x50, 0x38, 0x4c])) { // VP8L, lossless: 14 bits each, bit-packed
  ...
  if (has(u8, 0, [0xff, 0xd8, 0xff])) {
    ...
    while (i + 9 < u8.length) {
      ...
      if (isFrame) {
        return { width: (u8[i + 7] << 8) | u8[i + 8], height: (u8[i + 5] << 8) | u8[i + 6], frames: 1, format: 'JPEG' };
      }
      if (length < 2) break;
      i += 2 + length;
    }
...
export function inspect(u8, filename = '', limits = LIMITS) {
  ...
  if (u8.length > limits.maxBytes) {
    return {
      ok: false,
      reason: 'That file is ' + (u8.length / 1048576).toFixed(1) + ' MB, and the limit is ' +
        (limits.maxBytes / 1048576) + ' MB. An application mark is drawn a few dozen pixels across.'
    };
  }
  ...
  if (!ACCEPTED.has(found.mime)) {
    ...
    return {
      ok: false,
      reason: 'The bytes in that file are ' + found.type + ', not an image this can use.' +
        (claimed ? ' The name says .' + claimed.toLowerCase() + ', but an extension is a label, not the contents.' : ''),
  ...
  if (!dims.width || !dims.height) {
    return { ok: false, reason: 'That image reports a zero dimension, which no decoder can use.', sniffed: found.type };
  }
  if (dims.width > limits.maxDimension || dims.height > limits.maxDimension) {
    return {
      ok: false,
      reason: 'That image is ' + dims.width + ' by ' + dims.height + ' pixels, and the limit on either side is ' +
        limits.maxDimension + '.',
      sniffed: found.type
    };
  }
  if (dims.width * dims.height > limits.maxPixels) {
    return {
      ok: false,
      reason: 'That image is ' + (dims.width * dims.height / 1048576).toFixed(1) + ' megapixels, which is more than ' +
        'this will decode. Decoded, it would need roughly ' +
        ((dims.width * dims.height * 4) / 1048576).toFixed(0) + ' MB of memory.',
      sniffed: found.type
    };
  }
  ...
  const note = dims.frames > limits.maxFrames
    ? 'That image has ' + dims.frames + ' frames. An application mark is a still picture, so only the first is used.'
    : '';
...
export function safeAreaWarnings({ width, height, crop }) {
  ...
  if (c.size > 0.98 && Math.min(width, height) / Math.max(width, height) < 0.9) {
    warnings.push('The crop covers the whole of a non-square image, so the shorter side will be padded.');
  }
  return warnings;
}
...
export function verifyVariant(u8, expectedMime = 'image/png') {
  ...
  return { ok: true, width: dims.width, height: dims.height, bytes: u8.length, evidence: found.evidence };
}
...
export function contrastWarnings(averageRgb, surfaces, contrastFn) {
  ...
  for (const s of surfaces) {
    ...
    if (ratio < 1.6) {
      out.push('On the ' + s.label + ' the mark is at ' + ratio.toFixed(2) +
