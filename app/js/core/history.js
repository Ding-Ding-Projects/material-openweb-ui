 ...
 */
export const NEVER_RECORDED = Object.freeze(['secret', 'hash', 'password', 'pin', 'token', 'apiKey']);
...
export function redact(value, depth = 0) {
  ...
  if (typeof value === 'object') {
    ...
    for (const [k, v] of Object.entries(value)) {
      if (NEVER_RECORDED.includes(k)) { out[k] = '[not recorded]'; continue; }
      out[k] = redact(v, depth + 1);
    }
    return out;
  }
  if (typeof value === 'string' && value.length > LIMITS.maxValueBytes) {
    return value.slice(0, LIMITS.maxValueBytes) + '… [' + (value.length - LIMITS.maxValueBytes) + ' more characters not recorded]';
  }
  return value;
}
...
export async function record(log, { action, target, before, after, at, label }) {
  ...
  return { log: { schema: SCHEMA_VERSION, entries: next, truncated: log ? log.truncated || 0 : 0 }, entry, dropped: 0 };
}
...
export async function verify(log) {
  ...
  for (const entry of entries) {
    if ((entry.parent || '') !== parent) {
      return { ok: false, seq: entry.seq, reason: 'entry ' + entry.seq + ' does not follow the one before it' };
    }
    const expected = await hashOf(entry);
    if (expected !== entry.hash) {
      return { ok: false, seq: entry.seq, reason: 'entry ' + entry.seq + ' has been changed since it was written' };
    }
    parent = entry.hash;
  }
...
export function days(log) {
  ...
  return [...counts.entries()].map(([day, count]) => ({ day, count })).sort((a, b) => b.day.localeCompare(a.day));
}
...
export function filter(log, { test, action, from, to } = {}) {
  return log.entries.filter((e) => {
    ...
    if (test) {
      const text = [e.action, e.target, e.label, JSON.stringify(e.before), JSON.stringify(e.after)].join(' ');
...
export function diff(before, after) {
  ...
  if (!isObject(before) || !isObject(after)) {
    if (JSON.stringify(before) !== JSON.stringify(after)) {
      out.push({ key: '', kind: before === undefined ? 'added' : after === undefined ? 'removed' : 'changed', before, after });
    }
    return out;
  }
...
export async function prune(log, { before, keepLabelled = true, at } = {}) {
  ...
  const kept = log.entries.filter((e) => String(e.at).slice(0, 10) >= before || labelled.has(e.seq));
  ...
  return record({ schema: SCHEMA_VERSION, entries: rebuilt, truncated: log.truncated || 0 }, {
    action: 'prune',
    target: 'entries before ' + before,
    before: { entries: log.entries.length },
    after: { entries: rebuilt.length },
    at,
    label: removed + ' entries removed, ' + (keepLabelled ? 'keeping every labelled one' : 'including labelled ones')
  }).then((r) => ({ ...r, removed }));
}
...
export function forExport(log) {
  return {
    ...
    omitted: [
      'Any field named ' + NEVER_RECORDED.join(', ') + ' — these are replaced with "[not recorded]" when the entry is written, so they are absent from the history itself and not merely from this export.',
      log.truncated ? log.truncated + ' entries beyond the ' + LIMITS.maxEntries + '-entry cap were dropped as newer ones arrived.' : null
    ].filter(Boolean)
  };
}
