...
export function closeOthers(model, keepId) {
  ...
  return { model: { ...model, tabs: ordered(keep, model.groups), activeTab: keepId }, closed: model.tabs.length - keep.length };
}
...
export function setGroupColour(model, groupId, colour) {
  ...
  return { model: { ...model, groups: model.groups.map((g) => (g.id === groupId ? { ...g, colour } : g)) } };
}

export function setGroupCollapsed(model, groupId, collapsed) {
  return { model: { ...model, groups: model.groups.map((g) => (g.id === groupId ? { ...g, collapsed } : g)) } };
}
...
export function moveToGroup(model, tabId, groupId) {
  ...
  const tabs = model.tabs.map((t) => (t.id === tabId ? { ...t, group: groupId, pinned: groupId ? false : t.pinned } : t));
  return { model: { ...model, tabs: ordered(tabs, model.groups) } };
}
