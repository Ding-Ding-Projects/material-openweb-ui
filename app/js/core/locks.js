...
export function list() {
  return Object.entries(all()).map(([id, lock]) => ({ id, ...lock, secret: undefined, hash: undefined }));
}
...
export async function create({ id, label, method, password, secret, duration = 'surface' }) {
  ...
  if (method === 'password') {
    if (!password || password.length < 4) throw new Error('The password must be at least four characters.');
...
export async function tryUnlock(id, answer) {
  ...
  if (Date.now() < record.until) {
    ...
    return { ok: false, reason: 'Too many attempts. Try again in ' + secs + 's.', waitSeconds: secs };
  }
  ...
  if (!ok) {
    ...
    return {
      ok: false,
      reason: 'That did not match. ' + (record.until > Date.now()
  ...
  next[id] = {
    ...lock,
    openUntil: lock.duration === 'session' ? 'session' : Number.isFinite(minutes) ? Date.now() + minutes * 60_000 : Date.now()
  };
...
export function ladderBudget() {
  ...
  return { used: ladderUses.length, cap: LADDER_CAP, remaining: Math.max(0, LADDER_CAP - ladderUses.length) };
}
