...
function buildCodewords(text, version) {
  ...
  const blocks = [];
  let offset = 0;
  for (let i = 0; i < g1; i++) { blocks.push(data.slice(offset, offset + g1cw)); offset += g1cw; }
  for (let i = 0; i < g2; i++) { blocks.push(data.slice(offset, offset + g2cw)); offset += g2cw; }
...
export function toSvg(qr, { moduleSize = 6, quiet = 4 } = {}) {
  ...
  for (let r = 0; r < qr.size; r++) {
    for (let c = 0; c < qr.size; c++) {
      ...
      rects.push('<rect x="' + ((c + quiet) * moduleSize) + '" y="' + ((r + quiet) * moduleSize) +
...
export function decode(qr) {
  ...
  const unmasked = qr.modules.map((row, r) => row.map((v, c) =>
    isFunction(version, size, r, c) ? v : (fn(r, c) ? !v : v)));
  ...
  if (mode !== 0b0100) throw new Error('Only byte mode is supported by this decoder; got mode ' + mode.toString(2) + '.');
