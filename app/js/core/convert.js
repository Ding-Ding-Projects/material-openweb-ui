...
export function sniff(u8) {
	if (!u8 || !u8.length) {
		return {
			...
			evidence: 'The file contains no bytes.'
		};
	}
	...
	if (matches(u8, 0, [0x52, 0x49, 0x46, 0x46])) {
		if (matches(u8, 8, [0x57, 0x45, 0x42, 0x50]))
			return {
				type: 'WebP image',
				cat: 'Images',
				mime: 'image/webp',
				evidence: 'RIFF container tagged WEBP at offset 8'
			};
		if (matches(u8, 8, [0x57, 0x41, 0x56, 0x45]))
			return {
				type: 'WAV audio',
				cat: 'Audio',
				mime: 'audio/wav',
				evidence: 'RIFF container tagged WAVE at offset 8'
			};
		if (matches(u8, 8, [0x41, 0x56, 0x49, 0x20]))
			return {
				type: 'AVI video',
				cat: 'Video',
				mime: 'video/x-msvideo',
				evidence: 'RIFF container tagged AVI at offset 8'
			};
		return {
			type: 'RIFF container',
			cat: 'Binary Encodings',
			mime: 'application/octet-stream',
			evidence: 'RIFF container with an unrecognised inner tag'
		};
	}
	...
	if (matches(u8, 4, [0x66, 0x74, 0x79, 0x70])) {
		...
		if (/^(qt|M4A)/i.test(brand))
			return {
				type: 'MP4-family audio (' + brand.trim() + ')',
				cat: 'Audio',
				mime: 'audio/mp4',
				evidence: 'ISO base media brand "' + brand.trim() + '"'
			};
		return {
			type: 'MP4 video (' + brand.trim() + ')',
			cat: 'Video',
			mime: 'video/mp4',
			evidence: 'ISO base media brand "' + brand.trim() + '"'
		};
	...
	if (matches(u8, 0, [0x50, 0x4b, 0x03, 0x04]) || matches(u8, 0, [0x50, 0x4b, 0x05, 0x06])) {
		return {
			...
			evidence: 'ZIP local file header (PK\\x03\\x04)'
		};
	}
	...
	if (n && printable / n > 0.95) {
		...
		const evidence =
			Math.round((printable / n) * 100) + '% of the first ' + n + ' bytes are printable';
		if (text.startsWith('{') || text.startsWith('[')) {
			return {
				type: 'JSON / structured text',
				cat: 'Structured Data/Spreadsheets',
		...
		if (text.startsWith('<?xml'))
			return { type: 'XML document', cat: 'Code/Text', mime: 'application/xml', evidence };
		if (text.startsWith('<'))
			return { type: 'Markup text (HTML/SVG)', cat: 'Code/Text', mime: 'text/html', evidence };
...
export const ADAPTERS = [
	{
		id: 'img-png',
		cat: 'Images',
		from: ['image/png', 'image/jpeg', 'image/webp', 'image/gif', 'image/bmp'],
		to: 'PNG',
		available: true,
		kind: 'canvas',
		mime: 'image/png',
	...
	},
	{
		id: 'img-avif',
		cat: 'Images',
		from: ['image/png', 'image/jpeg', 'image/webp'],
		to: 'AVIF',
		available: false,
		reason:
			'No AVIF encoder is bundled. Chromium can decode AVIF but not encode it, so this needs a libavif build shipped inside the application.'
	},

	{
		id: 'pdf-make',
		cat: 'Documents/PDF',
		from: ['text/plain', 'text/html', 'application/json', 'text/csv'],
		to: 'PDF',
		available: false,
		reason:
			'Needs a bundled PDF writer (pdf-lib or equivalent). Nothing is installed, so this stays disabled rather than producing a file that only looks like a PDF.'
	},
	{
		id: 'pdf-text',
		cat: 'Documents/PDF',
...

	{
		id: 'audio-wav',
		cat: 'Audio',
		from: ['audio/mpeg', 'audio/wav', 'audio/flac', 'audio/ogg'],
		to: 'WAV (PCM)',
		available: false,
		reason:
			'Needs a bundled ffmpeg.wasm build. Not installed — and a system ffmpeg on PATH would not count, because it will not be there after installation.'
	},
	{
		id: 'video-webm',
		cat: 'Video',
		from: ['video/mp4', 'video/x-msvideo'],
		to: 'WebM',
		available: false,
	...
	},
	{
		...
		ext: 'json',
		discloses:
			'Every value is read as a string; numbers are not inferred, because guessing types is how leading zeros and long identifiers get destroyed.',
		destroys: []
	},
	{
		id: 'json-pretty',
		...
		destroys: [
			'Duplicate keys, which JSON permits but no parser keeps: they collapse to the last one.'
		]
	},

	{
		id: 'text-lf',
		cat: 'Code/Text',
		from: ['text/plain', 'text/html', 'application/json', 'text/csv', 'application/xml'],
		to: 'UTF-8 text (LF endings)',
		available: true,
		kind: 'textlf',
		mime: 'text/plain',
		ext: 'txt',
...

	{
		...
		mime: 'text/plain',
		ext: 'b64.txt',
		discloses: 'Output is about a third larger than the input.',
		destroys: []
	},
	{
		id: 'to-hex',
	...
	},
	{
		...
		cat: 'Binary Encodings',
		from: ['text/plain'],
		to: 'Bytes from Base64',
		available: true,
		kind: 'unb64',
		mime: 'application/octet-stream',
		ext: 'bin',
		discloses: 'Fails rather than guessing if the text is not valid Base64.',
		destroys: []
	}
];
...
export async function run(adapter, file, { decodeImage } = {}) {
	...
	if (!adapter.available)
		throw new Error(adapter.reason || 'That adapter is not available in this build.');
	...
	switch (adapter.kind) {
		case 'canvas': {
			if (!decodeImage)
				throw new Error(
					'Image conversion needs a renderer; this adapter cannot run outside a document.'
				);
		...
		case 'unb64': {
			...
			if (!/^[A-Za-z0-9+/]*={0,2}$/.test(cleaned)) {
				throw new Error(
					'That text is not valid Base64, so nothing was written. The source file is untouched.'
				);
			}
