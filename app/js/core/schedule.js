...
export const SCHEDULABLE = Object.freeze([
  'theme', 'language', 'funnyEn', 'funnyZh', 'narratorOn', 'appearanceProfile'
]);
...
function formatterFor(timeZone) {
  if (!partsCache.has(timeZone)) {
    partsCache.set(timeZone, new Intl.DateTimeFormat('en-GB', {
      timeZone,
      hour12: false,
...
export function validateRule(input, index = 0) {
  ...
  if (!id) throw new Error('Rule ' + index + ' has no identifier. Identifiers are stable and are what history refers to.');
  if (id.length > LIMITS.maxIdLength) throw new Error('Rule "' + id + '" has an identifier longer than ' + LIMITS.maxIdLength + ' characters.');
  if (!/^[a-zA-Z0-9._-]+$/.test(id)) throw new Error('Rule "' + id + '" has an identifier with characters outside letters, digits, dot, dash and underscore.');
  ...
  if (!days.length) throw new Error('Rule "' + id + '" applies to no days at all, so it could never run.');
  ...
  if (keys.length > LIMITS.maxPatchKeys) {
    throw new Error('Rule "' + id + '" changes ' + keys.length + ' settings, and the limit is ' + LIMITS.maxPatchKeys + '.');
  }
  for (const key of keys) {
    if (!SCHEDULABLE.includes(key)) {
      throw new Error('Rule "' + id + '" tries to change "' + key + '", which is not a setting a schedule may change.');
    }
    patch[key] = given[key];
  }
  ...
  return {
    id, name, startTime, endTime, startDate, endDate, days, patch,
...
export function validate(document) {
  ...
  if (document.schema !== SCHEMA_VERSION) {
    throw new Error('This build reads schedule schema version ' + SCHEMA_VERSION +
      ', and that document says ' + JSON.stringify(document.schema) + '.');
  }
  ...
  if (rules.length > LIMITS.maxRules) {
    throw new Error('That schedule has ' + rules.length + ' rules, and the limit is ' + LIMITS.maxRules + '.');
  }
  const seen = new Set();
...
export function evaluate(rule, instant, timeZone) {
  ...
  let startedOn;
  if (allDay) {
    startedOn = { year: now.year, month: now.month, day: now.day, weekday: now.weekday, date: now.date };
  } else if (!crossesMidnight) {
    if (now.minutes < start || now.minutes >= end) return { active: false, reason: 'outside the window' };
    startedOn = { year: now.year, month: now.month, day: now.day, weekday: now.weekday, date: now.date };
  ...
  if (!rule.days.includes(startedOn.weekday)) {
    return { active: false, reason: 'the window would have begun on ' + WEEKDAYS[startedOn.weekday] + ', which this rule does not cover' };
  }
  if (rule.startDate && startedOn.date < rule.startDate) return { active: false, reason: 'before the start date' };
  if (rule.endDate && startedOn.date > rule.endDate) return { active: false, reason: 'after the end date' };
...
export function resolve(document, instant = new Date(), timeZone) {
  ...
  return {
    settings,
    because,
    applied: ordered.map((e) => ({ id: e.rule.id, name: e.rule.name, specificity: specificity(e.rule), startedOn: e.startedOn })),
    timeZone: zone,
...
export function describe(rule, timeZone) {
  ...
  const dates = rule.startDate || rule.endDate
    ? ' between ' + (rule.startDate || 'the beginning') + ' and ' + (rule.endDate || 'no end date')
    : '';

  const midnight = crosses
    ? ' Because it crosses midnight, the window belongs to the day it starts on: a ' +
      WEEKDAYS[rule.days[0]] + ' window runs into the following morning without ' +
      WEEKDAYS[(rule.days[0] + 1) % 7] + ' needing to be selected.'
    : '';
