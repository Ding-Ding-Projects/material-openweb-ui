...
export const LIMITS_TEXT =
	...
	' MB, schema version ' +
	LIMITS.supportedVersions.join(' or ') +
	', at most ' +
	LIMITS.maxTerms.toLocaleString('en-US') +
	' terms, nesting no deeper than ' +
	LIMITS.maxDepth +
	', keys under ' +
	LIMITS.maxKeyLength +
	' characters and replacements under ' +
...
export function validate(parsed, byteLength) {
	if (typeof byteLength === 'number' && byteLength > LIMITS.maxBytes) {
		return {
			...
			error:
				'The file is ' +
				(byteLength / 1024 / 1024).toFixed(2) +
				' MB, over the ' +
				LIMITS.maxBytes / 1024 / 1024 +
				' MB limit.'
		};
	...
	if (!('version' in parsed)) {
		return {
			ok: false,
			error: 'There is no "version" field, so there is no way to know which schema this is.'
		};
	}
	...
	if (depth > LIMITS.maxDepth) {
		return {
			ok: false,
			error: 'The file nests ' + depth + ' levels deep, over the limit of ' + LIMITS.maxDepth + '.'
		};
	}
		...
		if (typeof replacement !== 'string') {
			return {
				...
				error:
					...
					' ("' +
					alias.slice(0, 24) +
					'") has a non-string replacement. Replacements must be strings.'
			};
		}
		...
		if (seen.has(alias)) {
			return {
				...
				error:
					'The alias "' +
					alias.slice(0, 32) +
					'" appears more than once, so which replacement wins is ambiguous.'
			};
		}
		seen.add(alias);
...
export async function load(file) {
	...
	try {
		...
		return {
			ok: false,
			error: 'That is not valid JSON: ' + String(e && e.message ? e.message : e)
		};
	}
