...
export async function fetchCatalog({ signal, source = 'https://ollama.com/library' } = {}) {
  ...
  while (queue.length) {
    ...
    if (pages >= MAX_PAGES) {
      ...
      return fail('Stopped after ' + MAX_PAGES + ' pages. The catalogue may go further; this is a bound on the request, not the end of the list.');
    }
    ...
    if (pages === 1 && names.size === before) {
      return fail('The catalogue page loaded but no model links could be read from it, which means its markup changed and this parser needs updating.');
    }
...
export function catalogView(fresh, cached) {
  ...
  if (fresh && fresh.models.length && !fresh.failure) {
    ...
    return {
      ...
      title: fresh.completeness === 'complete' ? 'Catalogue verified complete' : 'Catalogue read, completeness not verified',
      note: fresh.models.length + ' models across ' + pageWord + ', read just now. ' + (fresh.basis || '') +
        (fresh.revision ? ' Source revision ' + fresh.revision + ' (' + fresh.revisionFrom + ').' : '')
    };
  }
