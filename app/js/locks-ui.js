...
export function wizard(targetId, label) {
	...
	const secretInput = h('input', {
		...
		'aria-label': 'One-time-code secret'
	});
		...
		add(
			...
			method === 'password'
				...
				: h(
						...
						h(
							'div',
							{ class: 'muted', style: { fontSize: '.76rem', lineHeight: '1.55' } },
					...
					locks
						...
						.then(() => {
							ui.notify(
								'“' +
									label +
									'” is locked. The recovery route is in Support Tickets if the answer goes missing.',
								{ kind: 'ok' }
							);
			...
			add(
				...
				budget.remaining
					? h(
							'button',
							{
								...
								onclick: () => {
									...
									ladder(targetId, () => unlockPrompt(targetId, onOpened));
								}
							},
					...
					: h(
							'span',
							{},
							'The ladder is used up for this hour, so the clock is the only way through.'
						)
			);
		}
	}
...
export function ladder(targetId, onDone) {
	...
	if (!budget.remaining) {
		ui.notify(
			'The ladder has been used ' +
				budget.cap +
				' times this hour, which is its cap. The clock is the only way through now — that cap is what keeps this safe rather than clever.',
			{ kind: 'info', persist: true }
		);
		return null;
	}
	...
	function finishWon() {
		...
		dialog.close();
		ui.notify(
			'The wait is cleared — and only the wait. You still need the answer, because guessing a dumpling was never an authentication factor.',
	...
	function fallToClock(why) {
		...
		ui.notify(
			why +
				' The ladder is not offered again for this lockout, so you serve the wait you were already serving — which is exactly where you started, and no worse.',
			{ kind: 'info', persist: true }
		);
		if (onDone) onDone();
	}
	...
	function paintDish() {
		...
		add(
			...
			h(
				...
				'Five wrong and it moves on. ' + (5 - wrongDishes) + ' left.'
			)
		);
	}
		...
		function ask() {
			...
			add(
				...
				h(
					'div',
					{ class: 'muted', style: { fontSize: '.76rem' } },
					'Every one has to be right. One wrong and it moves on.'
				)
			);
	...
	function paintMole() {
		...
		function paintScore() {
			score.textContent =
				...
				NEED +
				' — ' +
				Math.max(0, Math.ceil((ROUND_MS - (Date.now() - startedAt)) / 1000)) +
				's left';
		}
...
export function supportTickets(targetId) {
	...
	const category = h(
		...
		...['Cannot remember the answer', 'Lost the authenticator', 'It was my sibling', 'Other'].map(
			(c) => h('option', { value: c }, c)
		)
	);
	const description = h('textarea', {
		rows: '3',
		'aria-label': 'Description',
		placeholder: 'Tell us what happened. Nobody will read it.',
		style: {
			width: '100%',
			borderRadius: '14px',
			padding: '10px 12px',
		...
		if (!all.length) {
			add(
				...
				h(
					...
					'No tickets have been raised on this machine.'
				)
			);
			return;
		}
		for (const t of all.slice(0, 6)) {
			add(
				...
				h(
					...
					h(
						'span',
						{ class: 'chip chip--tonal mono', style: { height: '22px', fontSize: '.64rem' } },
						t.number
					),
		...
		body: h(
			...
			h(
				...
				h(
					...
					h(
						'div',
						{ class: 'state__text', style: { fontWeight: '600' } },
				...
				'button',
				{
					...
					onclick: () => {
						...
						const t = {
							...
							at: Date.now(),
							status: 'Awaiting triage (forever)'
						};
						state.set('tickets', [t, ...(state.get('tickets') || [])].slice(0, 40));
						state.log('Support ticket raised', number);
						paintList();
						ui.notify('Ticket ' + number + ' raised. Severity: urgent. Assigned to: nobody.', {
							kind: 'info'
						});
					}
				},
			...
			h(
				...
				h(
					...
					'The only thing that actually works: clear ' +
						dataDir +
						'. That removes every lock, along with everything else this application has stored. It is your action, in your own file manager — this never deletes anything for you.'
				),
				h(
					'div',
						...
						'button',
						{
							...
							onclick: async () => {
								...
								if (info?.userData) {
									...
									ui.notify(
										'Opened ' + info.userData + '. Deleting it is your call, and your action.',
										{ kind: 'info', persist: true }
									);
								} else {
									ui.notify(
										"This is running in a browser, which has no application-data folder. Clearing this site's storage is the equivalent.",
										{ kind: 'info', persist: true }
									);
								}
							}
					...
					h(
						'button',
						{
							...
							onclick: async () => {
								const info = await (await import('./desktop.js')).appInfo();
								ui.copyToClipboard(
									info?.userData || 'browser storage for this origin',
									'Path copied.'
								);
							}
						},
