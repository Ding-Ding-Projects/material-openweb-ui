...
function ruleDialog(doc, existing, onDone) {
  const rule = existing || {
    id: newId(), name: '', startTime: '20:00', endTime: '07:00',
    startDate: '', endDate: '', days: [0, 1, 2, 3, 4, 5, 6], patch: { theme: 'dark' }, enabled: true
  };

  const name = h('input', { type: 'text', value: rule.name || '', placeholder: 'Evenings', 'aria-label': 'Rule name' });
  ...
  const startDate = h('input', { type: 'date', value: rule.startDate || '', 'aria-label': 'First day (optional)' });
  const endDate = h('input', { type: 'date', value: rule.endDate || '', 'aria-label': 'Last day (optional)' });
  const err = h('div', { style: { color: 'var(--err)', fontSize: '.8rem', minHeight: '18px' } });
  const preview = h('div', { class: 'sched__preview' });

  let days = [...rule.days];
  const dayRow = h('div', { class: 'sched__days', role: 'group', 'aria-label': 'Days of the week' });
  for (let d = 0; d < 7; d++) {
    const btn = h('button', {
      type: 'button',
  ...
  function paintPatch() {
    ...
    for (const [key, meta] of Object.entries(SETTING_CHOICES)) {
      ...
      const sel = ui.select({
        ...
        onChange: (v) => {
          patch[key] = key.startsWith('funny') ? Number(v) : v === 'true' ? true : v === 'false' ? false : v;
          paintPreview();
        }
      });
      const toggle = ui.toggle({
        ...
        onChange: (checked) => {
          if (checked) patch[key] = key.startsWith('funny') ? Number(meta.options[0]) : meta.options[0];
          else delete patch[key];
          paintPatch();
          paintPreview();
        }
      });
      add(patchRows, h('div', { class: 'sched__patchrow' },
        toggle.el,
  ...
  function paintPreview() {
    ...
    try {
      ...
      add(preview,
        ...
        h('div', { class: 'muted', style: { fontSize: '.72rem', marginTop: '4px' } },
          'Precedence weight ' + schedule.specificity(built) + '. A more specific rule wins; among equals, the one lower in the list wins.'));
    } catch (e) {
  ...
  const d = ui.dialog({
    ...
    body: h('div', { class: 'stack', style: { gap: '14px' } },
      ...
      h('div', { class: 'sched__times' },
        h('label', {}, h('span', {}, 'From'), startTime),
        h('label', {}, h('span', {}, 'To'), endTime)),
      h('p', { class: 'muted', style: { fontSize: '.76rem', lineHeight: '1.6' } },
    ...
    actions: [
      ...
      existing
        ...
        : null,
      { label: existing ? 'Save' : 'Add', primary: true, run: () => {
          let built;
          try {
            built = collect();
          } catch (e) {
...
export function render(box, onChanged) {
  ...
  for (const rule of doc.rules) {
    ...
    const winning = resolved
      ? Object.entries(resolved.because).filter(([, id]) => id === rule.id).map(([k]) => k)
      : [];
    add(list, h('div', { class: 'sched__rule' + (inForce ? ' sched__rule--live' : '') },
      h('div', { class: 'sched__rulehead' },
        h('strong', {}, rule.name),
        inForce
          ? h('span', { class: 'sched__badge' }, winning.length ? 'in force' : 'in force, overridden')
          : null,
        rule.enabled === false ? h('span', { class: 'sched__badge sched__badge--off' }, 'off') : null,
        h('span', { style: { flex: '1' } }),
        h('button', {
      ...
      h('div', { class: 'sched__rulepatch' },
        ...Object.entries(rule.patch).map(([k, v]) =>
          h('span', { class: 'sched__chip' + (winning.includes(k) ? ' sched__chip--winning' : '') },
            (SETTING_CHOICES[k] ? SETTING_CHOICES[k].label : k) + ' → ' + String(v))))));
  }

  if (!doc.rules.length) {
    add(list, h('div', { class: 'muted', style: { fontSize: '.82rem', padding: '10px 0' } },
      'No rules yet. Nothing changes on a schedule until one is added.'));
  }

  const zoneRow = h('div', { class: 'sched__zone' },
    icon('clock', 'icon icon--sm'),
    h('div', {},
      h('div', {}, 'Times are read in ' + doc.timeZone + '.'),
      resolved
        ? h('div', { class: 'muted', style: { fontSize: '.75rem' } },
            'It is ' + String(resolved.at.hour).padStart(2, '0') + ':' + String(resolved.at.minute).padStart(2, '0') +
            ' there now, and ' + resolved.applied.length + ' of ' + doc.rules.length + ' rules are in force.')
        : null));
      ...
      ? h('div', { class: 'state state--bad' }, icon('warn'),
          h('div', { class: 'state__body' },
            ...
            h('div', { class: 'state__text' },
              doc.broken + ' Your rules have not been deleted — they are simply not being applied until this is resolved.')))
      : null,
    zoneRow,
    list,
    h('div', { class: 'sched__actions' },
      h('button', { class: 'btn btn--filled', onclick: () => ruleDialog(doc, null, repaint) },
        icon('plus', 'icon icon--sm'), 'Add a rule'),
      doc.rules.length
        ? h('button', {
            class: 'btn btn--outlined',
            onclick: () => ui.downloadFile('schedule.json', JSON.stringify(doc, null, 2), 'application/json')
          }, 'Export the schedule')
        : null),
    h('details', { class: 'sched__dst' },
