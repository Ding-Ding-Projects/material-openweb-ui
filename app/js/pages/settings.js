...
function provenance(key, format) {
  ...
  const shown = format ? format(d) : typeof d === 'boolean' ? (d ? 'on' : 'off') : String(d === '' ? '(none)' : d);
...
function row({ id, label, why, control, prov }) {
  return h('div', { class: 'setting', id: id },
    h('div', { class: 'setting__main' },
      h('div', { class: 'setting__label' }, label),
      i18n.isBilingual() && i18n.t2(id) ? h('div', { class: 'cjk muted', style: { fontSize: '.78rem' } }, i18n.t2(id)) : null,
...
export function render(root) {
  ...
  if (!i18n.isPlayfulHidden('language')) {
    const sel = ui.select({
      value: state.get('settings').language, width: 200, label: i18n.t('set.language'),
      options: i18n.LANGUAGES.map((l) => ({ value: l, label: l })),
      onChange: (v) => set({ language: v }, 'language', 'Language mode → ' + v)
    });
    add(rows, row({ id: 'set.language', label: i18n.t('set.language'), why: i18n.t('set.languageWhy'), control: sel.el, prov: provenance('language') }));
  }

  for (const [key, labelKey] of [['funnyEn', 'set.funnyEn'], ['funnyZh', 'set.funnyZh']]) {
    if (i18n.isPlayfulHidden(key)) continue;
    const out = h('span', { class: 'mono muted', style: { fontSize: '.74rem', minWidth: '120px', textAlign: 'right' } }, i18n.levelLabel(state.get('settings')[key]));
    const input = h('input', {
      type: 'range', class: 'slider', min: '1', max: '5', step: '1',
      value: String(state.get('settings')[key]), 'aria-label': i18n.t(labelKey),
      style: { width: '140px' },
      oninput: (e) => { out.textContent = i18n.levelLabel(Number(e.target.value)); },
      onchange: (e) => set({ [key]: Number(e.target.value) }, key, i18n.t(labelKey) + ' → ' + e.target.value)
    });
    add(rows, row({
      id: labelKey, label: i18n.t(labelKey), why: i18n.t('set.funnyWhy'),
  ...
  if (!i18n.isPlayfulHidden('emojiDialogs')) {
    const tg = ui.toggle({
      checked: !!state.get('settings').emojiDialogs, label: i18n.t('set.emoji'),
      onChange: (v) => set({ emojiDialogs: v }, 'emojiDialogs', 'Emoji in dialogs → ' + (v ? 'on' : 'off'))
    });
    add(rows, row({ id: 'set.emoji', label: i18n.t('set.emoji'), why: i18n.t('set.emojiWhy'), control: tg.el, prov: provenance('emojiDialogs') }));
  }

  const themeSel = ui.select({
    value: state.get('settings').theme, width: 200, label: i18n.t('set.theme'),
    options: [
  ...
  add(rows, row({ id: 'set.theme', label: i18n.t('set.theme'), why: i18n.t('set.themeWhy'), control: themeSel.el, prov: provenance('theme') }));

  const hostInput = h('input', {
    type: 'text', class: 'mono', value: state.get('settings').ollamaHost, 'aria-label': i18n.t('set.host'),
    onchange: (e) => {
  ...
  add(rows, row({
    id: 'set.host', label: i18n.t('set.host'), why: i18n.t('set.hostWhy'),
  ...
  const vocabBox = i18n.isPlayfulHidden('vocab') ? null : h('div', { class: 'card', style: { marginTop: '20px' } });
  ...
  add(page,
    ...
    i18n.isPlayfulHidden('funnyEn')
      ? null
      : h('div', { class: 'state state--info', style: { marginBottom: '18px' } }, icon('info'),
          h('div', { class: 'state__body' },
            h('div', { class: 'state__title' }, 'About the funny level'),
            h('p', { class: 'state__text' }, i18n.FUNNY_DISCLOSURE.en),
            h('p', { class: 'state__text cjk', style: { marginTop: '6px', opacity: '.85' } }, i18n.FUNNY_DISCLOSURE.zh))),
    rows,
    narratorBox
  );
...
function renderNarrator(box, set) {
  clear(box);
  const n = state.get('settings').narrator || { on: false, voiceEn: '', voiceZh: '', rate: 1, pitch: 1 };

  const status = h('div', { class: 'muted', style: { fontSize: '.78rem', lineHeight: '1.6', marginTop: '10px' } });

  const toggle = ui.toggle({
    checked: !!n.on, label: 'Spoken narrator',
    onChange: (v) => {
  ...
  function paintPickers() {
    ...
    if (!narrator.supported()) {
      add(pickers, h('div', { class: 'muted', style: { fontSize: '.8rem' } },
    ...
    const langs = i18n.isPlayfulHidden('language')
      ? [['voiceEn', 'Voice', 'en']]
      : [['voiceEn', 'English voice', 'en'], ['voiceZh', 'Cantonese voice', 'zh']];
    ...
    for (const [key, langLabel, tag] of langs) {
      ...
      const options = [{ value: '', label: 'Choose automatically' },
        ...forLang.map((v) => ({ value: v.voiceURI, label: v.name + ' — ' + v.lang }))];

      const sel = ui.select({
        value: current, width: 280, label: langLabel, options,
        onChange: (v) => {
      ...
      add(pickers, h('div', { class: 'row', style: { gap: '14px', flexWrap: 'wrap' } },
        h('div', { style: { minWidth: '150px', fontSize: '.85rem', fontWeight: '600' } }, langLabel),
        sel.el,
        h('button', {
      ...
      if (current && !chosen) {
        add(pickers, h('div', { class: 'muted', style: { fontSize: '.75rem', color: 'var(--err)' } },
          'The chosen ' + langLabel.toLowerCase() + ' is not installed on this computer. The choice is being kept rather than reset, and the narrator falls back to an automatic pick until it returns.'));
      } else if (!forLang.length) {
        add(pickers, h('div', { class: 'muted', style: { fontSize: '.75rem' } },
          'No voice on this machine can read ' + (tag === 'zh' ? 'Cantonese' : 'English') + '. Installing one in the operating system makes it appear here.'));
      }
    }
  ...
  const rate = h('input', {
    type: 'range', class: 'slider', min: '0.5', max: '2', step: '0.1', value: String(n.rate ?? 1),
    'aria-label': 'Speech rate', style: { width: '140px' },
    onchange: (e) => {
      const cur = state.get('settings').narrator || {};
  ...
  const pitch = h('input', {
    type: 'range', class: 'slider', min: '0', max: '2', step: '0.1', value: String(n.pitch ?? 1),
    'aria-label': 'Speech pitch', style: { width: '140px' },
    onchange: (e) => {
      const cur = state.get('settings').narrator || {};
  ...
  add(box,
    h('div', { class: 'row', style: { gap: '12px', marginBottom: '6px' } },
      icon('bell'),
    ...
    n.on ? pickers : null,
    n.on ? h('div', { class: 'row', style: { gap: '20px', flexWrap: 'wrap', marginTop: '6px' } },
      h('div', { class: 'row', style: { gap: '10px' } }, h('span', { style: { fontSize: '.82rem' } }, 'Rate'), rate),
      h('div', { class: 'row', style: { gap: '10px' } }, h('span', { style: { fontSize: '.82rem' } }, 'Pitch'), pitch)) : null,
    status
  );
  ...
  if (n.on) {
    ...
    status.textContent = 'The narrator is off, so nothing is spoken. Turning it on reveals a voice picker for each language.';
  }
}
...
function renderVocabulary(box) {
  ...
  const input = h('input', {
    ...
    onchange: async (e) => {
      ...
      if (!result.ok) {
        ui.notify('That file was rejected and nothing changed: ' + result.error, { kind: 'error', persist: true });
      } else {
        state.set('vocabulary', result.value);
        state.log('Vocabulary loaded', result.value.terms.length + ' term(s)');
        ui.notify('Loaded ' + result.value.terms.length + ' term(s). Nothing about the file left this machine.', { kind: 'ok' });
      }
  ...
  add(box,
    h('div', { class: 'row', style: { gap: '12px', marginBottom: '6px' } },
      ...
      h('button', { class: 'btn btn--outlined', onclick: () => input.click() }, loaded ? 'Replace' : 'Choose a file'),
      loaded ? h('button', {
    ...
    loaded
      ? h('div', { class: 'state state--ok', style: { marginTop: '10px' } }, icon('check'),
          h('div', { class: 'state__body' },
            h('div', { class: 'state__title' }, loaded.terms.length + ' term(s) loaded'),
            h('div', { class: 'state__text' }, 'Schema version ' + loaded.version + '. Replacements apply to rendered copy only — commands, addresses, identifiers and file paths are never rewritten.')))
      : h('div', { class: 'state state--info', style: { marginTop: '10px' } }, icon('info'),
          h('div', { class: 'state__body' },
            h('div', { class: 'state__title' }, 'No file loaded'),
            h('div', { class: 'state__text' }, 'Every surface renders its original shipped wording. This control is present before a file exists on purpose — it is not permission to ship built-in mappings, samples or guesses.'))),
    h('details', { style: { marginTop: '12px' } },
      h('summary', { style: { cursor: 'pointer', fontSize: '.8rem', color: 'var(--p)' } }, 'What the file has to look like'),
      h('pre', { style: { marginTop: '10px' } }, vocab.EXAMPLE),
      h('p', { class: 'muted', style: { fontSize: '.78rem', marginTop: '8px', lineHeight: '1.6' } }, vocab.LIMITS_TEXT))
  );
}
...
function renderSchool(box, school) {
  ...
  const nameInput = h('input', {
    type: 'text', value: school.name || 'School mode', 'aria-label': 'What this mode is called',
    onchange: (e) => {
  ...
  function turnOn() {
    const pin = h('input', { type: 'password', inputmode: 'numeric', placeholder: '4 or more digits', 'aria-label': 'PIN' });
    ...
    ui.dialog({
      ...
      actions: [
        ...
        { label: 'Turn it on', primary: true, run: () => {
          const v = pin.value.trim();
          if (!/^\d{4,}$/.test(v)) { err.textContent = 'The PIN must be at least four digits.'; return true; }
          state.patchSettings({ school: { ...school, on: true, pin: v } });
          state.log('School mode', 'on');
          ui.notify((school.name || 'School mode') + ' is on. The playful surfaces are gone until it is turned off.', { kind: 'ok' });
          window.mowuiApp.refresh();
        } }
      ]
    });
  ...
  function turnOff() {
    const pin = h('input', { type: 'password', inputmode: 'numeric', placeholder: 'PIN', 'aria-label': 'PIN' });
    const err = h('div', { style: { color: 'var(--err)', fontSize: '.8rem', minHeight: '18px' } });
    ui.dialog({
      title: 'Turn off ' + (school.name || 'School mode'),
      emoji: '🔓',
      body: h('div', { class: 'stack', style: { gap: '12px' } },
        h('p', { class: 'muted', style: { fontSize: '.86rem' } }, 'Enter the PIN that was set when it was turned on. Your earlier choices were kept and come back with it.'),
        h('div', { class: 'field' }, pin),
        err,
        h('p', { class: 'muted', style: { fontSize: '.78rem', lineHeight: '1.6' } },
      ...
      actions: [
        { label: i18n.t('action.cancel') },
        { label: 'Turn it off', primary: true, run: () => {
          if (pin.value.trim() !== school.pin) { err.textContent = 'That PIN does not match.'; return true; }
          state.patchSettings({ school: { ...school, on: false } });
          state.log('School mode', 'off');
          ui.notify((school.name || 'School mode') + ' is off. Your earlier choices are back.', { kind: 'ok' });
          window.mowuiApp.refresh();
        } }
      ]
    });
  ...
  add(box,
    ...
    school.on ? null : h('div', { class: 'row', style: { gap: '12px', marginTop: '10px', flexWrap: 'wrap' } },
