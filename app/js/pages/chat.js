...
export function render(root) {
	const page = h('div', {
		class: 'page',
		style: { display: 'flex', flexDirection: 'column', height: '100%', maxWidth: '900px' }
	});
	const thread = h('div', {
	...
	const modelBtn = h(
		...
		icon('server', 'icon icon--sm'),
		h('span', {}, model || 'No model'),
		icon('arrow', 'icon icon--sm')
	);
	modelBtn.addEventListener('click', () => {
		if (!models.length) {
			ui.notify(
				'No models are installed, so there is nothing to choose. Pull one from the Ollama page first.',
				{ kind: 'info' }
			);
			return;
		}
	...
	const sendBtn = h(
		'button',
		{ class: 'btn btn--filled', onclick: () => submit() },
		icon('arrow', 'icon icon--sm'),
		'Send'
	);
	const stopBtn = h(
		'button',
		{ class: 'btn btn--outlined', style: { display: 'none' }, onclick: () => controller?.abort() },
		icon('x', 'icon icon--sm'),
		'Stop'
	);
	...
	function paint() {
		...
		if (!messages.length) {
			thread.append(
				h(
					'div',
					{ class: 'pending', style: { marginTop: '40px' } },
					icon('chat', 'icon icon--lg'),
					h('strong', {}, 'Nothing has been asked yet'),
					h(
						'span',
						{ class: 'muted', style: { fontSize: '.85rem', maxWidth: '54ch' } },
		...
		for (const m of messages) {
			thread.append(
				h(
					...
					h(
						'div',
						{ class: 'msg__who', dataset: { role: m.role } },
						m.role === 'user' ? 'You' : 'AI'
					),
	...
	async function submit() {
		...
		if (!model) {
			ui.notify(
				'Choose a model first — there is no default, because guessing which of your models you meant is not something this can do honestly.',
				{ kind: 'info' }
			);
			return;
		}
		...
		try {
			...
			state.log(
				'Chat reply',
				model + (result.tokensPerSecond ? ' · ' + result.tokensPerSecond.toFixed(1) + ' tok/s' : '')
			);
		} catch (e) {
			reply.streaming = false;
	...
	(async () => {
		try {
			...
			banner.append(
				h(
					'div',
					{ class: 'state state--bad', style: { marginBottom: '14px' } },
					icon('warn'),
					h(
						'div',
						{ class: 'state__body' },
						...
						h(
							...
							{ class: 'state__text' },
							e.message +
								' Nothing can be asked until it is running — and this surface will not answer with something it invented in the meantime.'
						),
						h(
							'button',
							{
								...
								onclick: () => window.mowuiApp.open('ollama')
							},
							'Open the Ollama page'
						)
					)
				)
			);
		...
		if (!models.length) {
			...
			banner.append(
				h(
					'div',
					{ class: 'state state--info', style: { marginBottom: '14px' } },
					icon('info'),
					h(
						'div',
						{ class: 'state__body' },
						...
						h(
							'div',
							{ class: 'state__text' },
							'The daemon is running and reported an empty list. Pull a model and it will appear in the picker.'
						),
						h(
							'button',
							{
	...
	page.append(
		h(
			'div',
			{ class: 'page__head', style: { flex: 'none' } },
			h(
				'div',
				{ style: { flex: '1' } },
