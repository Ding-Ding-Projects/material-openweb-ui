...
export function render(root) {
  ...
  const bar = bulkBar({
    ...
    exportRows: (ids) => {
      ...
      return allItems().filter((i) => want.has(i.id))
        .map((i) => ({ version: i.version, date: i.date, section: i.section, change: i.text, commit: i.sha }));
    }
  });
  ...
  function paint() {
    ...
    for (const [version, items] of byVersion) {
      ...
      const block = h('div', { class: 'card' },
        h('div', { class: 'chg__head' },
          h('span', { class: 'chg__version' }, version),
          release && release.codename ? h('span', { class: 'chg__codename' }, release.codename) : null,
          h('span', { style: { flex: '1' } }),
      ...
      for (const [title, sectionItems] of bySection) {
        ...
        for (const item of sectionItems) {
          add(block, h('div', { class: 'chg__item' },
  ...
  const sectionSel = ui.select({
    value: '', width: 190, label: 'Kind of change',
    options: [
      { value: '', label: 'Every kind (' + allItems().length + ')' },
      ...sections.map((s) => ({ value: s, label: s + ' (' + allItems().filter((i) => i.section === s).length + ')' }))
    ],
    onChange: (v) => { section = v; paint(); }
  });
  ...
  add(page,
    h('div', { class: 'page__head' },
      ...
      h('button', {
        ...
        onclick: () => {
          const rows = allItems().map((i) => ({ version: i.version, date: i.date, section: i.section, change: i.text, commit: i.sha }));
          ui.downloadFile('changelog.md', formats.serialise(rows, 'markdown'), 'text/markdown');
        }
      }, icon('download', 'icon icon--sm'), 'Export as Markdown')),
    h('div', { class: 'chg__filters' },
      field.el, sectionSel.el,
