...
export function render(root) {
	...
	function renderDetail() {
		...
		const extLies =
			ext &&
			!sniffed.mime.includes(ext) &&
			!(ext === 'jpg' && sniffed.mime === 'image/jpeg') &&
			!(ext === 'txt' && sniffed.mime === 'text/plain');
		...
		add(
			...
			h(
				...
				h(
					...
					{ class: 'kv' },
					h('dt', {}, 'Detected'),
					h('dd', {}, h('strong', {}, sniffed.type)),
					h('dt', {}, 'Category'),
					h('dd', {}, sniffed.cat),
					h('dt', {}, 'Media type'),
					h('dd', { class: 'mono' }, sniffed.mime),
					h('dt', {}, 'Evidence'),
					h('dd', { class: 'mono', style: { fontSize: '.74rem' } }, sniffed.evidence)
				),
				extLies
					? h(
							'div',
							...
							h(
								...
								h(
									...
									'This file is named .' +
										...
										sniffed.type +
										'. The bytes win: every adapter below is chosen from what the file actually is.'
								)
							)
						)
					: null
	...
	function renderCatalogue(mime) {
		...
		const field = searchField({
			...
			label: 'Search conversion adapters'
		});
		...
		function paint() {
			...
			for (const cat of convert.CATEGORIES) {
				const items = convert.ADAPTERS.filter((a) => a.cat === cat).filter((a) =>
					m.test(a.to + ' ' + a.cat + ' ' + (a.reason || '') + ' ' + (a.discloses || ''))
				);
					...
					h(
						...
						...items.map((a) => {
								...
								'button',
								{
									...
									title: !a.available
										...
										: !mime
											? 'Load a file first.'
											: !applies
												? 'This adapter does not accept ' + mime + '.'
												: 'Convert'
								},
							...
							return h(
								...
								{
									class: 'card',
									style: { padding: '14px 18px', opacity: a.available ? '1' : '.72' }
								},
								h(
									'div',
									...
									h(
											...
											convert.isLossy(a)
												? h(
														'span',
														{
															...
															style: { height: '20px', fontSize: '.62rem', padding: '0 8px' }
														},
														'lossy'
													)
												: null,
										...
										h(
											...
											a.available ? a.discloses : a.reason
										)
									),
									btn
								)
	...
	async function runOne(adapter) {
		...
		const go = async () => {
			try {
				...
				paintResults();
				ui.notify(
					'Converted to ' + name + ' (' + fmtBytes(blob.size) + '). The source file is untouched.',
					{ kind: 'ok' }
				);
			} catch (e) {
		...
		if (!list.length) {
			results.append(
				h(
					...
					'Nothing has been converted in this session yet.'
				)
			);
			return;
		}
		for (const r of list) {
			results.append(
				h(
					...
					h(
						'div',
						{ class: 'stack', style: { gap: '1px', flex: '1', minWidth: '0' } },
						h('strong', { style: { fontSize: '.85rem' } }, r.name),
						h(
							'span',
							{ class: 'muted', style: { fontSize: '.72rem' } },
							r.adapter + ' · ' + fmtBytes(r.size)
						)
					),
					r.url
						? h(
								'a',
								{ class: 'btn btn--outlined', href: r.url, download: r.name },
