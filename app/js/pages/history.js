...
export function render(root) {
  ...
  let from = '';
  let to = '';
  const field = searchField({ placeholder: 'Search the history…', label: 'Search history entries' });

  function visible() {
    return historyCore.filter(log(), { test: field.matcher().ok ? field.matcher().test : null, action, from, to });
  }
    ...
    actions: [
      {
        ...
        confirm: false,
        run: async (ids) => {
          const input = h('input', { type: 'text', placeholder: 'A note you will recognise later', 'aria-label': 'Label' });
          ui.dialog({
  ...
  async function paintChain() {
    ...
    add(chainRow,
      ...
      h('span', { class: 'muted hist__caveat' },
  ...
  function paint() {
    ...
    if (!all.entries.length) {
      add(list, h('div', { class: 'state state--info' }, icon('info'),
    ...
    if (!entries.length) {
      add(list, h('div', { class: 'muted', style: { padding: '14px 0', fontSize: '.84rem' } },
    ...
    for (const e of entries.slice().reverse()) {
      ...
      add(list, h('div', { class: 'hist__row' },
        ...
        h('div', { class: 'hist__body' },
          ...
          changes.length
            ? h('div', { class: 'hist__diff' }, ...changes.map((c) => h('div', { class: 'hist__change' },
          ...
          h('div', { class: 'hist__actions' },
            h('button', {
              ...
              onclick: async () => {
                ...
                ui.notify('Restored, and recorded as a new entry — the original is still there.', { kind: 'ok' });
                paint();
                paintChain();
              }
            }, icon('undo', 'icon icon--sm'), 'Restore this value'),
            h('button', {
              class: 'btn btn--outlined btn--sm',
  ...
  function filterRow() {
    ...
    const actionSel = ui.select({
      value: action, width: 210, label: 'Action',
      options: [
        { value: '', label: 'Every action (' + all.entries.length + ')' },
        ...acts.map((a) => ({ value: a.action, label: a.action + ' (' + a.count + ')' }))
      ],
      onChange: (v) => { action = v; paint(); }
    });

    const fromSel = ui.select({
      value: from, width: 190, label: 'From',
      options: [{ value: '', label: 'The beginning' }, ...dayList.slice().reverse().map((d) => ({ value: d.day, label: d.day + ' (' + d.count + ')' }))],
      onChange: (v) => { from = v; paint(); }
    });
    const toSel = ui.select({
      value: to, width: 190, label: 'To',
      options: [{ value: '', label: 'Today' }, ...dayList.map((d) => ({ value: d.day, label: d.day + ' (' + d.count + ')' }))],
      onChange: (v) => { to = v; paint(); }
    });
  ...
  function pruneDialog() {
    ...
    const daySel = ui.select({
      value: before, width: 210, label: 'Remove entries before',
      options: dayList.slice().reverse().map((d) => ({ value: d.day, label: d.day })),
      onChange: (v) => { before = v; }
    });
    const keep = ui.toggle({
      checked: true, label: 'Keep labelled entries',
      onChange: (v) => { keepLabelled = v; }
    });
    ...
    ui.dialog({
      ...
      body: h('div', { class: 'stack', style: { gap: '12px' } },
        h('p', { class: 'muted', style: { fontSize: '.82rem', lineHeight: '1.6' } },
        ...
        daySel.el,
        h('div', { class: 'row', style: { gap: '10px', alignItems: 'center' } }, keep.el, h('span', {}, 'Keep labelled entries')),
        h('p', { class: 'muted', style: { fontSize: '.78rem' } },
          'The chain is rebuilt across what remains, so it still verifies afterwards.')),
      actions: [
        { label: 'Cancel' },
        { label: 'Prune', danger: true, run: async () => {
          const result = await historyCore.prune(log(), { before, keepLabelled });
          state.set('history', result.log);
          ui.notify(result.removed + ' entries removed, and the removal recorded.', { kind: 'ok' });
          paint();
          paintChain();
        } }
      ]
  ...
  add(page,
    h('div', { class: 'page__head' },
      ...
      h('div', { class: 'row', style: { gap: '8px' } },
        h('button', { class: 'btn btn--outlined', onclick: pruneDialog }, icon('trash', 'icon icon--sm'), 'Prune…'),
        h('button', {
          ...
          onclick: () => {
            ...
            ui.dialog({
              ...
              actions: [
                { label: 'Cancel' },
                { label: 'Save as JSON', primary: true, run: () =>
                  ui.downloadFile('history.json', formats.serialise(out.rows, 'json'), 'application/json') }
              ]
            });
          }
        }, icon('download', 'icon icon--sm'), 'Export all'))),
    chainRow,
    filterRow(),
    bar.el,
    list);
