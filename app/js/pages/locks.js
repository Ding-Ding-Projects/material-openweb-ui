...
export function render(root) {
  ...
  const field = searchField({
    placeholder: 'Search locks…', label: 'Search locks',
  ...
  function paint() {
    ...
    for (const l of rows) {
      ...
      const cb = h('input', {
        type: 'checkbox', 'aria-label': 'Select ' + l.label, style: { accentColor: 'var(--p)' },
        checked: selected.has(l.id),
        onchange: (e) => { e.target.checked ? selected.add(l.id) : selected.delete(l.id); sync(); }
      });

      add(list, h('div', { class: 'card', style: { display: 'flex', gap: '12px', alignItems: 'center', padding: '14px 18px' } },
        cb,
        ...
        h('div', { class: 'stack', style: { gap: '2px', flex: '1', minWidth: '0' } },
          ...
          h('span', { class: 'muted', style: { fontSize: '.74rem' } },
            ...
            ' · created ' + fmtTime(l.createdAt) +
            ' · ' + (open ? 'open' : 'locked'))),
        open
          ? h('button', { class: 'btn btn--outlined', onclick: () => { locks.relock(l.id); paint(); } }, 'Lock again')
          : h('button', { class: 'btn btn--outlined', onclick: () => locksUi.unlockPrompt(l.id, paint) }, 'Unlock'),
        h('button', {
          class: 'btn btn--text', style: { color: 'var(--err)' },
          onclick: () => ui.superConfirm({
            what: 'Remove the lock on “' + l.label + '”',
            affects: 'The lock and its credential are removed from this machine. The element stops being locked. Nothing else is touched, and nothing you have stored is deleted.',
            onConfirm: () => { locks.remove(l.id); selected.delete(l.id); paint(); sync(); }
          })
        }, 'Remove')));
    }
  }
  ...
  function sync() {
    ...
    const rows = view();
    add(bulkBar,
      h('button', { class: 'btn btn--text', onclick: () => { rows.forEach((r) => selected.add(r.id)); paint(); sync(); } },
        'Select all ' + rows.length + ' matching'),
      h('button', { class: 'btn btn--text', onclick: () => { rows.forEach((r) => selected.has(r.id) ? selected.delete(r.id) : selected.add(r.id)); paint(); sync(); } }, 'Invert'),
      h('button', { class: 'btn btn--text', onclick: () => { selected.clear(); paint(); sync(); } }, 'Clear'),
      selected.size ? h('span', { class: 'chip' }, selected.size + ' selected') : null,
      selected.size ? h('button', {
        class: 'btn btn--danger',
        onclick: () => ui.superConfirm({
          what: 'Remove ' + selected.size + ' lock(s)',
          affects: 'Each selected lock and its credential is removed from this machine. The elements stop being locked. Nothing you have stored is deleted.',
          onConfirm: () => {
      ...
      }, icon('trash', 'icon icon--sm'), 'Remove selected') : null);
  }

  field.onChange(() => { paint(); sync(); });
  ...
  add(page,
    h('div', { class: 'page__head' },
      ...
      h('button', { class: 'btn btn--outlined', onclick: () => locksUi.supportTickets() }, icon('file', 'icon icon--sm'), 'Support Tickets')),
