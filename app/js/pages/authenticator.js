...
export function render(root) {
	...
	function addDialog() {
		...
		const digitSel = ui.select({
			value: digits,
			options: [
				{ value: 6, label: '6 digits (usual)' },
		...
		const periodInput = h('input', {
			...
			oninput: (e) => {
				...
				if (Number.isFinite(v) && v > 0) period = Math.round(v);
			}
		});
		const periodField = h(
			'label',
			{ class: 'field-labelled' },
			h('span', {}, 'Period (seconds)'),
			periodInput
		);
		...
		const d = ui.dialog({
			...
			body: h(
						...
						'button',
						{
							...
							onclick: () => {
								try {
									...
									digits = p.digits;
									period = p.period;
									algoSel.set(algorithm);
									digitSel.set(digits);
									periodInput.value = String(period);
									err.textContent = '';
									err.style.color = 'var(--ok)';
									err.textContent =
										'Read: ' + p.algorithm + ', ' + p.digits + ' digits, ' + p.period + 's period.';
								} catch (e) {
				...
				h(
					'div',
					{ class: 'grid grid--2', style: { gap: '10px' } },
				...
				h(
					'div',
					{ class: 'row', style: { gap: '10px', flexWrap: 'wrap', alignItems: 'flex-end' } },
					algoSel.el,
					digitSel.el,
					periodField
				),
			...
			actions: [
				{ label: 'Cancel' },
				{
					...
					run: () => {
						...
						if (!s) {
							err.style.color = 'var(--err)';
							err.textContent = 'A secret is required.';
							return true;
						}
						try {
							totp.base32Decode(s).length ||
								(() => {
						...
						entries.push({
							...
							account: account.value.trim() || 'account',
							secret: s,
							algorithm,
							digits,
							period
	...
	function pairDialog() {
		...
		const confirm = h('input', {
			type: 'text',
			class: 'mono',
			placeholder: 'Type the current code',
			'aria-label': 'Confirmation code'
		});
		const err = h('div', { style: { color: 'var(--err)', fontSize: '.8rem', minHeight: '18px' } });
		const qrBox = h('div', {
			style: { background: '#FFFFFF', padding: '10px', borderRadius: '12px', width: 'fit-content' }
		});
		const secretBox = h(
			'div',
			{
		...
		function paintQr() {
			...
			qrBox.firstChild.setAttribute(
				'aria-label',
				'A QR code pairing ' +
					(account.value.trim() || 'this account') +
					' with ' +
					(issuer.value.trim() || 'this application') +
		...
		const d = ui.dialog({
			...
			body: h(
				...
				h(
					'div',
					{ class: 'row', style: { gap: '20px', alignItems: 'flex-start', flexWrap: 'wrap' } },
					qrBox,
					h(
						'div',
						{ class: 'stack', style: { gap: '8px', flex: '1', minWidth: '220px' } },
						h('strong', { style: { fontSize: '.85rem' } }, 'Or type it in'),
						secretBox,
						h(
							'div',
							{ class: 'muted', style: { fontSize: '.75rem' } },
							'Click to reveal. SHA-1, 6 digits, 30-second period.'
						),
						h(
							'button',
							{
								class: 'btn btn--outlined',
								onclick: () => ui.copyToClipboard(secret, 'Secret copied.')
							},
							'Copy the secret'
						)
					)
				...
				h(
					...
					h(
						'p',
						{ class: 'muted', style: { fontSize: '.8rem', lineHeight: '1.55' } },
			...
			actions: [
				{ label: 'Cancel' },
				{
					...
					run: () => {
						...
						Promise.all(
							[-1, 0, 1].map((w) => totp.totp(secret, { atMs: Date.now() + w * 30000 }))
						).then((allowed) => {
							if (!allowed.includes(confirm.value.replace(/\s+/g, ''))) {
								err.textContent =
									'That code does not match. Give the authenticator a moment to finish adding it, then type the code it is showing now.';
								return;
							}
							entries.push({
								id: idFor(),
	...
	async function paint() {
		...
		for (const e of matched) {
			...
			try {
				...
				/* an invalid secret shows dashes rather than throwing */
			}
			...
			list.append(
				h(
					'div',
					{ class: 'card', dataset: { entry: e.id } },
					h(
						'div',
						{ class: 'row', style: { gap: '14px', flexWrap: 'wrap' } },
						h(
							'div',
							{ class: 'stack', style: { gap: '2px', flex: '1', minWidth: '160px' } },
						...
						h(
							'div',
							{ class: 'stack', style: { gap: '2px', alignItems: 'flex-end' } },
							h(
								'div',
								{ class: 'code-lg', 'aria-live': 'polite', 'aria-label': 'Current code' },
								groupCode(code)
							),
						...
						h(
							...
							h(
								...
								{
									class: 'btn btn--outlined',
									onclick: () => ui.copyToClipboard(code, 'Code copied.')
								},
								'Copy'
							),
							h(
								'button',
								{
									...
									onclick: () =>
										ui.superConfirm({
											...
											affects:
												'This entry and its secret are removed from memory. If you have no other copy of the secret, the account it belongs to becomes unreachable through this application.',
											onConfirm: () => {
	...
	const selfCheck = h(
		...
		h('div', { class: 'muted' }, 'Running the published test vectors…')
	);
	(async () => {
		...
		selfCheck.append(
			h(
				'div',
				{ class: 'state ' + (ok ? 'state--ok' : 'state--bad') },
				icon(ok ? 'check' : 'warn'),
				h(
					'div',
					{ class: 'state__body' },
	...
	function paintClock() {
			...
			lines.push(
				h(
					...
					h(
						'span',
						{},
						'The system clock moved ' +
							Math.abs(localJumpSeconds) +
							' seconds ' +
		...
		if (networkSkew === null) {
			...
			const when =
				ageMin < 1 ? 'just now' : ageMin === 1 ? '1 minute ago' : ageMin + ' minutes ago';
			lines.push(
				h(
					'div',
					{},
					Math.abs(networkSkew) <= 5
						? 'The system clock was within ' +
								Math.abs(networkSkew) +
								's of network time when checked ' +
								when +
								'.'
						: 'The system clock was ' +
								Math.abs(networkSkew) +
								's ' +
								(networkSkew > 0 ? 'ahead of' : 'behind') +
								' network time when checked ' +
								when +
								'. Codes will be refused until it is corrected — this is the failure nobody diagnoses, because the digits look perfectly fine.'
				)
			);
		}
	...
	const onFocus = () => {
		if (document.body.contains(page)) checkNetwork(false);
	};
	...
	page.append(
		h(
			...
			h(
				...
				h(
					'button',
					{ class: 'btn btn--outlined', onclick: pairDialog },
					icon('phonelock', 'icon icon--sm'),
					'Pair with a QR'
				),
				h(
					'button',
					{ class: 'btn btn--filled', onclick: addDialog },
