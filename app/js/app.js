...
const tabExtras = {
	...
	items(anchor, t) {
		...
		return [
			existing
				? {
						label: locksCore.isLocked(lockId) ? 'Unlock this tab…' : 'Lock it again',
						icon: 'lock',
						run: () =>
							locksCore.isLocked(lockId)
								? locksUi.unlockPrompt(lockId, render)
								: (locksCore.relock(lockId), render())
					}
				: {
						label: 'Lock this tab…',
						icon: 'lock',
...
function buildTitlebar() {
	...
	const controls = h(
		...
		h(
			'button',
			{
				'aria-label': 'Minimise',
				title: 'Minimise',
				onclick: () => desktop.windowControls.minimize()
			},
			svg('<path d="M2 6h8"/>')
		),
		h(
			...
			svg(
				maximised
					? '<rect x="2" y="4" width="6" height="6"/><path d="M4 4V2h6v6h-2"/>'
	...
	titlebar.append(
		...
		h(
			...
			state.get('settings').displayName || 'Material Open WebUI'
		),
...
function buildRail() {
		...
		rail.appendChild(
			h(
				...
				h(
					...
					{},
					pageLabel(id),
					bilingual() && pageLabel2(id)
						? h('span', { class: 'rail__zh cjk' }, pageLabel2(id))
						: null
				)
			)
		);
	...
	rail.appendChild(
		h(
			'div',
			{ class: 'muted', style: { fontSize: '.64rem', padding: '0 14px 6px', lineHeight: '1.5' } },
...
function exportAll() {
	...
	ui.notify('Exported. Authenticator secrets were left out, and the file lists what it omitted.', {
		kind: 'ok',
		persist: true
	});
}
...
state.subscribe((key, value) => {
	...
	narrator.say(
		latest.event + (latest.detail ? '. ' + latest.detail : ''),
		i18n.isCantonese() ? 'zh' : 'en'
	);
});
...
desktop.onEvent((event) => {
	if (event?.type === 'window:maximize') {
		maximised = true;
		buildTitlebar();
	}
	if (event?.type === 'window:unmaximize') {
		maximised = false;
		buildTitlebar();
	}
});
