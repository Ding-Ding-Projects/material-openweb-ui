	...
	for (const [key, record] of Object.entries(parsed.overrides || {})) {
		...
		for (const [stateId, props] of Object.entries(record || {})) {
			...
			for (const [propId, value] of Object.entries(props || {})) {
				if (!known.has(propId) || typeof value !== 'string' || value.length > 200) {
					dropped++;
					continue;
				}
...
export async function fonts() {
	...
	try {
		...
		fontCache = {
			...
			note:
				families.length +
				' typefaces installed on this computer, plus the faces this application ships.'
		};
	} catch (e) {
		fontCache = {
			families: [...bundled, ...generic],
	...
	function control(prop) {
			...
			current
				? h(
						'button',
						{
							type: 'button',
							class: 'ap__reset',
							title: 'Reset just this property',
							onclick: () => {
								resetProperty(key, stateId, prop.id);
								paint();
							}
						},
						icon('undo', 'icon icon--xs'),
						'Reset'
					)
				: null
		);
		...
		if (prop.kind === 'range') {
			...
			field = ui.select({
				...
				onChange: (v) => {
					setProperty(key, stateId, prop.id, v || null);
					paint();
				}
			}).el;
			...
			field = ui.select({
				...
				options: [
					{ value: '', label: 'Inherited' },
					...ELEVATIONS.map((e) => ({ value: e.value, label: e.label }))
				],
				onChange: (v) => {
					setProperty(key, stateId, prop.id, v || null);
					paint();
			...
			input.addEventListener('input', () => {
				preview.style.fontFamily = input.value || 'inherit';
			});
	...
	function paint() {
			...
			add(
				...
				h(
					'button',
					{
						type: 'button',
						role: 'tab',
						class: 'ap__state' + (st.id === stateId ? ' ap__state--on' : ''),
					...
					st.label,
					setCount ? h('span', { class: 'ap__count' }, String(setCount)) : null
				)
			);
		}
		...
		add(
			...
			h(
				...
				h(
					'button',
					{ type: 'button', class: 'ap__close', 'aria-label': 'Close', onclick: close },
					icon('x', 'icon icon--sm')
				)
			),
			exact
			...
			stateTabs,
			h('div', { class: 'ap__scroll' }, ...groupControls()),
			h(
				'div',
				{ class: 'ap__foot' },
				h(
					...
					{ class: 'muted' },
					count
						? count + ' set on ' + STATES.find((s) => s.id === stateId).label.toLowerCase()
						: 'Nothing overridden'
				),
				h(
					'button',
	...
	document.addEventListener(
		...
		(e) => {
			...
			const el = e.target.closest(
				'[data-ap], button, a, input, h1, h2, h3, p, li, td, th, .card, .btn'
			);
			if (!el) return;
			e.preventDefault();
			e.stopPropagation();
