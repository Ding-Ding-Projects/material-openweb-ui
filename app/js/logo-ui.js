...
function canvasBytes(canvas) {
	return new Promise((resolve, reject) => {
		canvas.toBlob((blob) => {
			if (!blob) {
				reject(new Error('The canvas produced no image at all.'));
				return;
			}
...
export function render(box, onChanged) {
	...
	function paintPreview() {
		...
		if (current.kind === 'preset') {
			...
			add(
				preview,
				h('img', { src: current.dataUrl, alt: 'The current application mark', class: 'logo__mark' })
			);
		}
	...
	for (const preset of logo.PRESETS) {
		const btn = h('button', {
			...
			class:
				'logo__preset' +
				(current.kind === 'preset' && current.preset === preset.id ? ' logo__preset--on' : ''),
			...
			add(
				...
				h(
					...
					h(
						...
						verdict.evidence
							? h('div', { class: 'state__text mono logo__evidence' }, verdict.evidence)
							: null
					)
				)
			);
				...
				h(
					...
					h(
						...
						h(
							'div',
							{ class: 'state__text' },
							'The header read as a valid ' +
								verdict.format +
								', but the decoder could not finish it: ' +
								(e && e.message ? e.message : 'no reason given') +
								'. Nothing was changed.'
						)
					)
				)
	...
	async function paintPending() {
		...
		for (const size of logo.VARIANT_SIZES) {
			...
			add(
				...
				h(
					'div',
					{ class: 'logo__variant' + (verified.ok ? '' : ' logo__variant--bad') },
					h(
						'div',
						{
							class: 'logo__variant-box',
							style: { width: Math.min(size, 64) + 'px', height: Math.min(size, 64) + 'px' }
						},
						verified.ok
							? h('img', {
									src: canvas.toDataURL('image/png'),
									alt: size + ' pixel variant',
									width: String(Math.min(size, 64)),
									height: String(Math.min(size, 64))
								})
		...
		for (const line of logo.safeAreaWarnings({
			width: bitmap.width,
				...
				h(
					...
					h(
						...
						h(
							'div',
							{ class: 'state__title' },
							failed.length + ' of ' + results.length + ' sizes did not verify'
						),
						h(
							'div',
							{ class: 'state__text' },
				...
				h(
					'button',
					{
						...
						onclick: () => {
							...
							ui.notify('The mark is applied. The application identity is unchanged.', {
								kind: 'ok'
							});
							pending = null;
							render(box, onChanged);
	...
	function cropControls(bitmap) {
		...
		for (const c of controls) {
			const readout = h(
				'span',
				{ class: 'logo__readout mono' },
				Math.round(pending.crop[c.id] * 100) + '%'
			);
			const input = h('input', {
				type: 'range',
				min: String(c.min),
			...
			add(
				...
				h(
					'label',
					{ class: 'logo__croprow' },
					h('span', { class: 'logo__croplabel' }, c.label),
					input,
					readout
				)
			);
	...
	function commitName() {
		...
		if (!next) {
			nameErr.textContent =
				'A name is required. Leaving it empty would give the window no title at all.';
			return;
		}
		nameErr.textContent = '';
