...
const NOTATIONS = ['hex', 'hex8', 'rgb', 'hsl', 'hsv', 'hwb', 'lab', 'lch', 'oklab', 'oklch', 'cmyk'];
 ...
 */
export function colourPicker({ value = '#6750A4', onChange, background = [1, 1, 1], withAlpha = true } = {}) {
  let hsv;
  let alpha;
  let notation = 'hex';
  ...
  field.addEventListener('pointermove', (e) => { if (dragging) setFromField(e.clientX, e.clientY); });
  ...
  function track({ label, className, max, get, set, format }) {
    ...
    let held = false;
    el.addEventListener('pointerdown', (e) => { held = true; el.setPointerCapture(e.pointerId); el.focus(); fromPointer(e.clientX); });
    el.addEventListener('pointermove', (e) => { if (held) fromPointer(e.clientX); });
    el.addEventListener('pointerup', (e) => { held = false; if (el.hasPointerCapture(e.pointerId)) el.releasePointerCapture(e.pointerId); });
    el.addEventListener('keydown', (e) => {
      const step = (e.shiftKey ? 10 : 1) * (max / 360 > 0.5 ? 1 : max / 100);
      const moves = {
        ArrowLeft: () => get() - step, ArrowDown: () => get() - step,
        ArrowRight: () => get() + step, ArrowUp: () => get() + step,
        Home: () => 0, End: () => max
      };
      if (!moves[e.key]) return;
      e.preventDefault();
  ...
  function commitEntry() {
    ...
    if (!colour.inGamut(read.rgb)) {
      ...
      entryErr.textContent = 'Outside what sRGB can show, by ' +
        Math.round(colour.overshoot(read.rgb) * 100) + '%. It will be applied as the nearest colour this display can produce.';
    }
  ...
  entry.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); commitEntry(); } });
  ...
  function paintFormats(f) {
    ...
    for (const key of NOTATIONS) {
      const text = f[key];
      add(allFormats, h('button', {
        type: 'button',
        class: 'cp__fmt',
        title: 'Copy ' + key.toUpperCase(),
        onclick: () => ui.copyToClipboard(text, key.toUpperCase() + ' copied.')
      }, h('span', { class: 'cp__fmt-key' }, key), h('span', { class: 'cp__fmt-val mono' }, text)));
    }
    if (f.named) {
      add(allFormats, h('button', {
        type: 'button', class: 'cp__fmt',
        onclick: () => ui.copyToClipboard(f.named, 'Name copied.')
      }, h('span', { class: 'cp__fmt-key' }, 'name'), h('span', { class: 'cp__fmt-val mono' }, f.named)));
    }
  }
  ...
  function paint() {
    ...
    add(contrastRow,
      ...
      h('span', { class: 'cp__grade cp__grade--' + (ratio >= 4.5 ? 'ok' : ratio >= 3 ? 'warn' : 'bad') },
        colour.grade(ratio)),
  ...
  const el = h('div', { class: 'cp' },
    field,
    h('div', { class: 'cp__rows' },
      h('div', { class: 'cp__row' }, swatch, hueTrack.el),
      withAlpha ? h('div', { class: 'cp__row' }, h('div', { class: 'cp__spacer' }), alphaTrack.el) : null),
    h('div', { class: 'cp__entry' }, notationSel.el, h('div', { class: 'field cp__entry-field' }, entry)),
    entryErr,
    contrastRow,
    h('details', { class: 'cp__details' },
      h('summary', {}, 'Every notation'),
      allFormats,
      h('p', { class: 'muted cp__note' },
