...
async function probeGpuWindows(): Promise<{ name: Fact<string>; vram: Fact<number>; note?: string }> {
  ...
  try {
    const { stdout } = await run('nvidia-smi', ['--query-gpu=name,memory.total', '--format=csv,noheader,nounits'], { timeout: 4000 });
  ...
  try {
    ...
    if (name) {
      return {
        name: known(name),
        vram: unknown('The adapter reports its memory through a 32-bit field that wraps above 4 GB, so its value is not trustworthy. Install nvidia-smi, or read the figure from the driver control panel.'),
...
export async function probe(modelDestination?: string): Promise<HardwareProbe> {
  ...
  let vramBytes: Fact<number> = unknown('GPU probing is only implemented for Windows in this build.');
  ...
  let freeDiskBytes: Fact<number> = unknown('No model destination was supplied, so free space was not measured.');
  if (modelDestination) {
    try {
      const s = await statfs(modelDestination);
      freeDiskBytes = known(Number(s.bavail) * Number(s.bsize));
    } catch (e) {
      freeDiskBytes = unknown('Free space on ' + modelDestination + ' could not be read: ' + String(e));
    }
  }
  ...
  return {
    ...
    platform: platform(),
    arch: arch(),
    cpuModel: cpuList.length ? known(cpuList[0].model.trim()) : unknown('The operating system reported no CPU information.'),
    cpuCores: cpuList.length ? known(cpuList.length) : unknown('The operating system reported no CPU information.'),
...
export function fit(
  hw: HardwareProbe,
  model: { blobBytes?: number; parameterCount?: number; quantisation?: string; contextTokens?: number }
): FitResult {
  ...
  if (!model.blobBytes || !Number.isFinite(model.blobBytes) || model.blobBytes <= 0) {
    return {
      ...
      assumptions: ['A size is never inferred from a model name or parameter count, so no verdict is offered.'],
  ...
  if (hw.freeDiskBytes.known && hw.freeDiskBytes.value < weights) {
    return {
      verdict: 'Unlikely',
      evidence: [...evidence, 'Free space on the model destination is ' + gb(hw.freeDiskBytes.value) + ', which is less than the download itself.'],
      assumptions,
      probedAt: hw.probedAt
    };
  }
  ...
  if (vram !== null) {
    ...
    if (needed <= vram * 0.85) {
      return { verdict: 'Runs well', evidence: [...evidence, 'Fits in GPU memory with headroom.'], assumptions, probedAt: hw.probedAt };
    }
    if (needed <= vram) {
      return { verdict: 'Runs with limits', evidence: [...evidence, 'Fits in GPU memory, but with little headroom for a long context.'], assumptions, probedAt: hw.probedAt };
    }
  ...
  if (needed <= ram * 0.5) {
    return { verdict: 'Runs with limits', evidence: [...evidence, 'Comfortably within system memory, but CPU inference is much slower than GPU.'], assumptions, probedAt: hw.probedAt };
  }
  if (needed <= ram * 0.8) {
    return { verdict: 'Unlikely', evidence: [...evidence, 'Would use most of system memory, leaving little for anything else running.'], assumptions, probedAt: hw.probedAt };
  }
  return { verdict: 'Unlikely', evidence: [...evidence, 'Needs more memory than this machine has.'], assumptions, probedAt: hw.probedAt };
}
